...
function modelSwitcher(models, selectedModel, idPrefix, customModels, forceDark = false) {
  ...
  return `<button id="${idPrefix}-model-switcher-button" class="relative w-full cursor-pointer rounded-md border ${
  ...
  } dark:text-gray-500">Model</label>
  <span class="inline-flex w-full truncate font-semibold ${
    forceDark ? 'text-gray-100' : 'text-gray-800'
  } dark:text-gray-100">
    <span class="flex h-6 items-center gap-1 truncate"><span id="${idPrefix}-selected-model-title">${
      selectedModel.title
    } ${selectedModel.tags
      ?.map(
        (tag) =>
          `<span class="py-0.25 mr-1 rounded px-1 text-sm capitalize bg-blue-200 text-blue-500">${tag}</span>`,
      )
      .join('')}</span>
    </span>
  </span>
...
<ul id="${idPrefix}-model-list-dropdown" style="width:400px;max-height:400px" class="hidden absolute z-10 mt-1 overflow-auto rounded-md py-1 text-base ring-1 ring-opacity-5 focus:outline-none ${
    forceDark ? 'bg-gray-800 ring-white/20 last:border-0' : 'bg-white'
  } dark:bg-gray-800 dark:ring-white/20 dark:last:border-0 sm:text-sm -translate-x-1/4" role="menu" aria-orientation="vertical" aria-labelledby="model-switcher-button" tabindex="-1">
  ${createModelListDropDown(models, selectedModel, idPrefix, customModels, forceDark)}
</ul>
<div style="width:200px; left:400px;" id="${idPrefix}-model-specifics" class="hidden absolute z-10 mt-1 ml-1 overflow-auto rounded-md p-2 text-base ring-1 ring-opacity-5 focus:outline-none ${
    forceDark ? 'bg-gray-800 ring-white/20 last:border-0' : 'bg-white'
  } dark:bg-gray-800 dark:ring-white/20 dark:last:border-0 sm:text-sm -translate-x-1/4">
asd
</div>`;
}
function createModelListDropDown(models, selectedModel, idPrefix, customModels, forceDark = false) {
  return `${models
    .map(
      ...
      } dark:border-white/20" id="${idPrefix}-model-switcher-option-${
        model.slug
      }" role="option" tabindex="-1">
 <div class="flex flex-col">
   <span class="font-semibold flex h-6 items-center gap-1 truncate ${
     forceDark ? 'text-gray-100' : 'text-gray-800'
   } dark:text-gray-100">${model.title} ${model?.tags
     ?.map(
       (tag) =>
         `<span class="py-0.25 mr-1 rounded px-1 text-sm capitalize bg-blue-200 text-blue-500">${tag}</span>`,
     )
     .join('')}</span>
   <span class="${forceDark ? 'text-gray-500' : 'text-gray-800'} dark:text-gray-500 text-xs">${
     model.description
   }</span>
 </div>
 ${
   model.slug === selectedModel.slug
     ? `<span id="${idPrefix}-model-switcher-checkmark" style="right:36px;" class="absolute inset-y-0 right-4 flex items-center ${
         forceDark ? 'text-gray-100' : 'text-gray-800'
       } dark:text-gray-100">
...
function createModelSpecifics(rating, forceDark = false) {
  ...
  const filledPills = [...Array(filled)].map(
    () => '<div class="h-2 w-full rounded-lg ml-1 bg-green-600"></div>',
  );
  const emptyPills = [...Array(total - filled)].map(
    () =>
      `<div class="h-2 w-full ml-1 rounded-lg ${
        forceDark ? 'bg-gray-600' : 'bg-gray-200'
      } dark:bg-gray-600"></div>`,
  );
...
function addModelSwitcherEventListener(idPrefix, forceDark = false) {
  ...
  const modelSwitcherOptions = document.querySelectorAll(
    `[id^=${idPrefix}-model-switcher-option-]`,
  );
  ...
  modelSwitcherOptions.forEach((option) => {
    option.addEventListener('mousemove', () => {
      ...
      chrome.storage.local.get(['models', 'unofficialModels', 'customModels'], (result) => {
        ...
        if (specifics && Object.keys(specifics).length > 0) {
          modelSpecifics.innerHTML = Object.keys(specifics)
            .map(
              (key) =>
                `<div class="flex items-center justify-between"><div class="text-sm capitalize ${
                  forceDark ? 'text-gray-100' : 'text-gray-800'
                } dark:text-gray-100">${key}</div><div class="flex w-1/2">${createModelSpecifics(
                  specifics[key],
                  forceDark,
                )}</div></div>`,
            )
      ...
      deleteButton.addEventListener('click', (e) => {
        ...
        chrome.storage.local.get(['customModels'], (result) => {
          ...
          chrome.storage.local.set({ customModels: newCustomModels }, () => {
            const modelSwitcherOption = document.querySelector(
              `[id$=-model-switcher-option-${slug}]`,
            );
    ...
    option.addEventListener('click', () => {
      chrome.storage.local.get(
        ...
        ({ settings, models, unofficialModels, customModels }) => {
          ...
          const pluginsDropdownWrapper = document.querySelector(
    ...
    chrome.storage.onChanged.addListener((e) => {
      if (e.settings && e.settings.newValue.selectedModel !== e.settings.oldValue.selectedModel) {
        const modelListDropdown = document.querySelector(`#${idPrefix}-model-list-dropdown`);
        modelListDropdown.classList.replace('block', 'hidden');
        const modelSwitcherCheckmark = document.querySelector(
          `#${idPrefix}-model-switcher-checkmark`,
        );
        ...
        selectedModelTitle.innerHTML = `${selectedModel.title} ${selectedModel.tags
          ?.map(
            (tag) =>
              `<span class="py-0.25 mr-1 rounded px-1 text-sm capitalize bg-blue-200 text-blue-500">${tag}</span>`,
          )
          .join('')}`;
        const selectedModelOption = document.querySelector(
          `#${idPrefix}-model-switcher-option-${selectedModel.slug}`,
        );
