...
function createPromptLibraryModal() {
  chrome.storage.local.get(['settings']).then((result) => {
    const selectedLibrarySortBy = result.settings.selectedLibrarySortBy || {
      name: 'New',
      code: 'recent',
    };
    const selectedLibraryLanguage = result.settings.selectedLibraryLanguage || {
      name: 'All',
      code: 'all',
    };
    const selectedLibraryCategory = result.settings.selectedLibraryCategory || {
      name: 'All',
      code: 'all',
    };
    ...
    // fetch data
    getPrompts(
      promptLibraryPageNumber,
      promptLibrarySearchTerm,
      selectedLibrarySortBy.code,
      selectedLibraryLanguage.code,
      selectedLibraryCategory.code,
    ).then((data) => {
      promptLibraryMaxPageNumber =
        data.count % 24 === 0 ? data.count / 24 : Math.floor(data.count / 24) + 1;
      ...
      createModal(
        'Community Prompts',
        `Find prompts shared by the community, and share your own prompts (${data.count} Prompts)`,
        bodyContent,
        actionsBarContent,
      );
  ...
  libraryItemTexts.forEach((libraryItemText) => {
    ...
    if (!promptLibrarySearchTerm || dataHidden) {
      ...
      if (libraryItemText.offsetHeight < libraryItemText.scrollHeight) {
        ...
        const existingLibraryItemReadMore = libraryList?.querySelector(
          `div[id=library-item-read-more-${id}]`,
        );
        ...
        libraryItemReadMore.style =
          'color: lightslategray; font-size:0.8em; width: 100%; margin-top: 8px; cursor: pointer;';

        libraryItemReadMore.textContent = dataHidden
          ? 'This is a private prompt. Full prompt is hidden by the autor.'
          : 'Show more';
        libraryItemReadMore.addEventListener('click', () => {
          if (dataHidden) return;
          if (libraryItemReadMore.textContent === 'Show less') {
            libraryItemText.style =
              'color: #ececf1; font-size:0.8em; width: 100%; white-space: break-spaces; overflow-wrap: break-word;display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical; overflow: hidden;';
            libraryItemReadMore.textContent = 'Show more';
          } else {
            libraryItemText.style =
              'color: #ececf1; font-size:0.8em; width: 100%; white-space: break-spaces; overflow-wrap: break-word;';
...
function promptLibraryListComponent(libraryData, loading = false) {
  ...
  libraryList.style =
    'display: flex; flex-direction: column; justify-content: start; align-items: center; height: 100%; width:100%;padding: 16px;overflow-y: scroll;';
  if (loading) {
  ...
  if (libraryData.results.length === 0) {
    ...
    noResult.style =
      'display: flex; justify-content: center; align-items: center; height: 340px; width: 100%;color: lightslategray;';
  ...
  libraryData.results.forEach((libraryPrompt) => {
    ...
    libraryItem.style =
      'position:relative; display: flex; flex-direction: column; justify-content: space-between; align-items: start; width: 100%; margin: 8px 0; padding: 8px; background-color: #1f2123; border-radius: 8px;';
    ...
    libraryItemActionWrapper.style =
      'position:absolute; top: 12px; right:4px; display: flex; justify-content: flex-end; align-items: center;';
    // thumbs up
    ...
    libraryItemTitle.style =
      'color: lightslategray; font-size:1em; font-weight: bold; width: 85%; white-space: break-spaces; overflow-wrap: break-word;margin-bottom: 8px;';
    ...
    libraryItemText.style = `color: #ececf1; font-size:0.8em; width: 100%; white-space: break-spaces; ${
      promptLibrarySearchTerm && !libraryPrompt.hide_full_prompt
        ? ''
        : 'overflow-wrap: break-word;display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical; overflow: hidden;'
    }`;
    // highlight searched text
    libraryItemText.innerHTML = highlightBracket(
      highlight(libraryPrompt.text, promptLibrarySearchTerm),
    );
    ...
    libraryItemCategories.style =
      'display:flex; justify-content: flex-start; align-items:center; width: 100%; margin-top: 12px;';
    // categories
    libraryPrompt.categories.forEach((category) => {
      const libraryItemCategory = document.createElement('span');
      libraryItemCategory.style =
        'background-color: #0b0d0e; color:lightslategray; font-size:11px; border-radius: 4px; padding: 2px 8px; margin-right: 8px;cursor: pointer;';
      ...
      libraryItemCategory.addEventListener('click', () => {
        const libraryCategoryOption = document.getElementById(
          `library-category-selector-option-${category.toLowerCase().replace(/ /g, '_')}`,
        );
    ...
    libraryItemFooter.style =
      'display:flex; justify-content: space-between; align-items:flex-end; width: 100%; white-space: break-spaces; overflow-wrap: break-word;margin-top: 8px';
    // created by url
    ...
    libraryItemInfoWrapper.style =
      'display: flex; align-items:flex-end; justify-content: start; color: lightslategray; font-size:0.8em; width: 100%; white-space: break-spaces; overflow-wrap: break-word;';
    ...
    libraryItemCreatedByUrl.style =
      'color: #919dd4; text-decoration:underline;max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;';
    libraryItemCreatedByUrl.innerHTML = highlight(
      libraryPrompt.created_by?.nickname,
      promptLibrarySearchTerm,
    );
    ...
    libraryItemThumbsUp.id = `library-item-thumbs-up-${libraryPrompt.id}`;
    libraryItemThumbsUp.title = 'Upvote this prompt';
    libraryItemThumbsUp.style =
      'color: lightslategray; font-size:1.2em; margin-left: 24px; position:relative; bottom:4px; cursor: pointer;';
    libraryItemThumbsUp.innerHTML =
    ...
    libraryItemThumbsUp.addEventListener('click', () => {
      vote(libraryPrompt.id, 'up').then((data) => {
        if (data.status === 'success') {
          toast('Prompt upvoted');
          const curUpvoteCount = document.getElementById(
            `prompt-upvotes-count-${libraryPrompt.id}`,
          );
          curUpvoteCount.textContent = parseInt(curUpvoteCount.textContent, 10) + 1;
        }
        if (data.status === 'same user') {
      ...
      const curLibraryItemActionWrapper = document.getElementById(
        `library-item-action-wrapper-${libraryPrompt.id}`,
      );
      curLibraryItemActionWrapper.style.opacity = '0.3';
      curLibraryItemActionWrapper.style.pointerEvents = 'none';
    });
    ...
    libraryItemThumbsDown.innerHTML =
      '<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"> <path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>';
    libraryItemThumbsDown.addEventListener('mouseenter', () => {
      libraryItemThumbsDown.style.color = '#eee';
    });
    ...
    libraryItemShareButton.style =
      'display:flex;color: lightslategray; margin-left: 24px;position: relative;bottom:3px;cursor: pointer;';
    ...
    libraryItemActionButtons.style =
      'display: flex; flex-direction: row; width: 100%; justify-content: end; align-items: end;margin-top:8px;position:relative;';
    const shiftClickText = document.createElement('div');
    shiftClickText.textContent = 'Shift + Click to run the prompt without editing';
    shiftClickText.style =
      'font-size:10px;position:absolute;right:0px;bottom:36px;display:none;color:lightslategray;';
    // use button
    ...
    libraryItemUseButton.style =
      'font-size:0.7em; padding:4px 8px; margin-left:8px;width:60px;color:lightgray;';
    libraryItemUseButton.textContent = 'Use';
    libraryItemUseButton.addEventListener('mouseover', () => {
      shiftClickText.style =
        'font-size:10px;position:absolute;right:0px;bottom:36px;color:lightslategray;';
    });
    ...
    libraryItemEditButton.style =
      'font-size:0.7em; padding:4px 8px; margin-left:8px;width:60px;color:lightgray;';
    libraryItemEditButton.textContent = 'Edit';
    libraryItemEditButton.addEventListener('click', () => {
      openSubmitPromptModal(
        libraryPrompt.text,
        libraryPrompt.model_slug,
        libraryPrompt.id,
        libraryPrompt.title,
        libraryPrompt.categories,
        libraryPrompt.language,
        libraryPrompt.hide_full_prompt,
      );
    });
    // delete button
...
function openReportPromptModal(libraryPrompt) {
  ...
  reasonSelectorWrapper.innerHTML = dropdown(
    'Report-Reason',
    reportReasonList,
    reportReasonList[0],
    'left',
  ...
  addDropdownEventListener(
    'Report-Reason',
    reportReasonList,
    (selectedOption) => toggleReportSubmitButton(selectedOption),
    true,
  );
  // add cancel submit button
  const reportModalButtonWrapper = document.createElement('div');
  reportModalButtonWrapper.style =
    'display:flex;justify-content:flex-end;align-items:center;width:100%;margin-top:32px;';
  ...
  reportModalCancelButton.style =
    'font-size:0.875rem;font-weight:500;padding:8px 16px;margin-right:16px;';
...
function promptLibraryModalContent(libraryData) {
  ...
  content.style =
    'display: flex; flex-direction: column; justify-content: space-between; align-items: center;overflow-y: hidden;height:100%;';
  const libraryList = promptLibraryListComponent(libraryData);
  // library filter
  const libraryFilterElement = document.createElement('div');
  libraryFilterElement.style =
    'display: flex; flex-direction: row; justify-content: space-between; align-items: center; width: 100%; padding: 6px 12px; background-color: #778899; z-index: 100; position: sticky; top: 0;';

  // add library search box
  const librarySearchInput = document.createElement('input');
  librarySearchInput.type = 'search';
  librarySearchInput.style =
    'background-color: #1f2123; color: lightslategray; font-size:0.8em; border-radius: 4px; border: 1px solid lightslategray; padding:4px 8px; width: 100%;height:46px;';
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    sortBySelectorWrapper.innerHTML = dropdown(
      'Library-SortBy',
      sortByList,
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    categorySelectorWrapper.innerHTML = dropdown(
      'Library-Category',
      categoryList,
  ...
  chrome.storage.local.get(['settings'], (result) => {
    ...
    languageSelectorWrapper.innerHTML = dropdown(
      'Library-Language',
      libraryLanguageList,
      selectedLibraryLanguage,
      'right',
      true,
    );
  ...
  pageButtonsWrapper.style =
    'display: flex; flex-direction: row; flex-wrap:wrap;justify-content: center; align-items: center;margin:8px 0;';
  const pageNumberElement = document.createElement('span');
  pageNumberElement.id = 'library-page-number';
  pageNumberElement.style =
    'color: lightslategray; font-size:0.8em; width: 100%; text-align: center;';
