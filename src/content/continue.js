...
function promptDropdown() {
  ...
  dropdown.style =
    'max-height:300px;overflow-y:scroll;width:132px;bottom:40px; left:0;z-index:200;';
  dropdown.classList =
    'hidden absolute z-10 right-0 mt-1 overflow-auto rounded-sm py-1 text-base ring-1 ring-opacity-5 focus:outline-none bg-white dark:bg-gray-800 dark:ring-white/20 dark:last:border-0 sm:text-sm -translate-x-1/4';
  ...
  chrome.storage.local.get(['customPrompts'], (result) => {
    ...
    for (let i = 0; i < allPrompts.length; i += 1) {
      ...
      dropdownItem.classList =
        'text-gray-900 relative cursor-pointer select-none border-b p-2 last:border-0 border-gray-100 dark:border-white/20 hover:bg-gray-500/10';
...
function addContinueButton() {
  ...
  continueButtonDropdown.style =
    'width:38px;border-top-right-radius:0;border-bottom-right-radius:0;z-index:2;';
  ...
  shiftClickText.style =
    'font-size:10px;position:absolute;left:0px;bottom:40px;display:none;color:lightslategray;width:200px;';
  const continueButton = document.createElement('button');
  chrome.storage.local.get('customPrompts', ({ customPrompts }) => {
    continueButton.textContent = Array.isArray(customPrompts)
      ? customPrompts.find((p) => p.isDefault)?.title || 'Continue'
      : 'Continue';
  });
  ...
  continueButton.style =
    'width:96px;border-radius:0;border-left:0;z-index:1;text-transform: capitalize;';
      ...
      textAreaElement.value = textAreaElement.value
        ? `${textAreaElement.value} ${
            Array.isArray(customPrompts)
              ? customPrompts.find((p) => p.isDefault)?.text || ''
              : 'Continue please'
          }`
        : `${
            Array.isArray(customPrompts)
  ...
  continueButton.addEventListener('mouseover', () => {
    shiftClickText.style =
      'font-size:10px;position:absolute;left:0px;bottom:40px;color:lightslategray;width:200px;';
  });
  ...
  autoClickButton.style =
    'width:38px;border-top-left-radius:0;border-bottom-left-radius:0;border-left:0;z-index:1;padding:0;';
  chrome.storage.local.get('settings', ({ settings }) => {
    autoClickButton.title = `Auto Continue is ${settings.autoClick ? 'ON' : 'OFF'}`;
    if (settings.autoClick) {
      autoClickButton.classList.add(
        'btn',
        'flex',
        'justify-center',
        'gap-2',
  ...
  autoClickButton.addEventListener('click', () => {
    chrome.storage.local.get('settings', ({ settings }) => {
      chrome.storage.local.set(
        { settings: { ...settings, autoClick: !settings.autoClick } },
        () => {
          if (settings.autoClick) {
          ...
          toast(
            `Auto Continue is ${
              settings.autoClick
                ? 'Disabled'
                : 'Enabled (<a style="text-decoration:underline; color:gold;" href="https://www.notion.so/ezi/Superpower-ChatGPT-FAQ-9d43a8a1c31745c893a4080029d2eb24?pvs=4#ed16d04d414941d4abcb59b6d765008d" target="blank">Learn More</a>)'
            }`,
            'success',
          );
        },
