...
function toggleOriginalRegenerateResponseButton() {
  ...
  const originalRegenerateResponseButton = allButtons.find(
    (button) => button.textContent.toLowerCase() === 'regenerate',
  );
  if (originalRegenerateResponseButton) {
    originalRegenerateResponseButton.remove();
  }
  const originalContinueGeneratingButton = allButtons.find(
    (button) => button.textContent.toLowerCase() === 'continue',
  );
  ...
  newRegenerateResponseButton.classList = `btn flex justify-center gap-2 ${
    textAreaElement.parentElement.style.display === 'none' ? 'btn-primary' : 'btn-neutral'
  } border`;
  ...
  newRegenerateResponseButton.addEventListener('click', () => {
    ...
    chrome.storage.local.get(['conversations', 'settings', 'models'], (result) => {
      ...
      while (
        lastUserMessageWrapper.nextElementSibling &&
        lastUserMessageWrapper.nextElementSibling.id.startsWith('message-wrapper-')
      ) {
  ...
  newContinueGeneratingButton.classList = `btn flex justify-center gap-2 ${
    textAreaElement.parentElement.style.display === 'none' ? 'btn-primary' : 'btn-neutral'
  } border`;
  newContinueGeneratingButton.innerHTML =
    '<svg stroke="currentColor" fill="none" stroke-width="1.5" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-3 w-3 -rotate-180" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polygon points="11 19 2 12 11 5 11 19"></polygon><polygon points="22 19 13 12 22 5 22 19"></polygon></svg> Continue';
  ...
  const erroMessageHTML =
    '<div id="input-form-error" class="w-full"><div class="mb-3 text-center text-xs">There was an error generating a response</div></div>';
