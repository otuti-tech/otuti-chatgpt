...
function createPromptHistoryModal() {
  chrome.storage.local.get(['userInputValueHistory', 'settings'], (result) => {
    ...
    createModal(
      'My Prompt History',
      'All your personal and favorite prompts are saved here.',
      bodyContent,
      actionsBarContent,
    );
...
function emptyHistory() {
  ...
  historyListEmpty.style =
    'display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100%; width: 100%; color: lightslategray; font-size: 13px; padding: 16px 0;text-align:center;';
  chrome.storage.local.get(['settings'], (result) => {
    const { settings } = result;
    const { historyFilter } = settings;
    const searchValue = document.querySelector('input[id="history-search-input"]').value;
    historyListEmpty.innerHTML =
      historyFilter === 'favorites'
        ? `No favorite found. ${
            searchValue
              ? 'Adjust your search'
              : `<span style="text-align:center;">You can mark any prompt in your history as favorite. Click on the <span style="text-decoration: underline;">All</span> tab above and then mark any prompt as favorite by clicking on the bookmark icon next to it <img src=${chrome.runtime.getURL(
                  'icons/bookmark-off.png',
                )} style="min-width: 24px; height: 32px; transform: rotate(90deg); position: relative; right: -10px; top: 2px;margin:auto;">`
          }</span>`
        : `No history found. ${
            searchValue
              ? 'Adjust your search'
              : 'Start using the chat to see your prompt history here'
          }`;
  });
...
function promptHistoryModalContent(userInputValueHistory, historyFilter) {
  ...
  content.style =
    'display: flex; flex-direction: column; justify-content: space-between; align-items: center;overflow-y: hidden;';
...
function promptHistoryList(userInputValueHistory, historyFilter) {
  ...
  if (userInputValueHistory && userInputValueHistory.length > 0) {
    userInputValueHistory
      ...
      .forEach((userInputValue, index) => {
        const historyItem = document.createElement('div');
        historyItem.id = `history-item-${index}`;
        historyItem.style =
          'display: flex; flex-direction: column; justify-content: space-between; align-items: end; width: 100%; margin: 8px 0; padding: 8px; background-color: #1f2123; border-radius: 8px;';
        ...
        historyItemActionButtons.style =
          'position:relative; display: flex; flex-direction: row; width: 100%; justify-content: end; align-items:end;margin-top:24px;';
        historyItem.appendChild(historyItemActionButtons);
        // submit button
        const historyItemSubmitButton = document.createElement('button');
        historyItemSubmitButton.classList =
          'btn flex justify-center gap-2 btn-dark border-0 md:border';
        historyItemSubmitButton.style =
          'font-size:0.7em; padding:4px 8px; margin-left:8px;color:lightgray;';
        ...
        historyItemDeleteButton.classList =
          'btn flex justify-center gap-2 btn-dark border-0 md:border';
        historyItemDeleteButton.style =
        ...
        historyItemDeleteButton.addEventListener('click', (e) => {
          if (e.target.textContent === 'Confirm') {
            chrome.storage.local.get(['userInputValueHistory'], (res) => {
              const newHistory = res.userInputValueHistory.filter(
                (item) => item.inputValue !== userInputValue.inputValue,
              );
        ...
        shiftClickText.style =
          'font-size:10px;position:absolute;right:32px;bottom:36px;display:none;color:lightslategray;';
        historyItemUseButton.classList =
          'btn flex justify-center gap-2 btn-dark border-0 md:border';
        historyItemUseButton.style =
          'font-size:0.7em; padding:4px 8px; margin-left:8px;width:60px;color:lightgray;';
        historyItemUseButton.textContent = 'Use';
        // add hover style to button
        historyItemUseButton.addEventListener('mouseover', () => {
          shiftClickText.style =
            'font-size:10px;position:absolute;right:32px;bottom:36px;color:lightslategray;';
        });
        ...
        historyItemBookmarkButton.style =
          'background-color: transparent; border: none; cursor: pointer; margin-left:8px;';
        const historyItemFavoriteButtonIcon = document.createElement('img');
        historyItemFavoriteButtonIcon.style =
          'min-width: 24px;height: 32px;transform: rotate(90deg);position: relative;right: -10px;top:2px;filter: drop-shadow(2px 4px 6px black);';
        historyItemFavoriteButtonIcon.src = chrome.runtime.getURL(
          `icons/${userInputValue.isFavorite ? 'bookmark-on' : 'bookmark-off'}.png`,
        );
        historyItemBookmarkButton.title = userInputValue.isFavorite
...
function historyFilterButtonsContent(historyFilter) {
  ...
  historyFilterElement.style =
    'display: flex; flex-direction: row; justify-content: space-between; align-items: center; width: 100%; padding: 12px; background-color: #778899; z-index: 100; position: sticky; top: 0;';
  // add history filter button
  const historyFilterButtons = document.createElement('div');
  historyFilterButtons.style =
  ...
  historyFilterFavoritesButton.style = `${
    historyFilter === 'favorites'
      ? 'background-color: gold; color: rgb(31, 33, 35);'
      : 'background-color: rgb(31, 33, 35); color: lightslategray;'
  } font-size:0.8em; cursor: pointer;border-top-left-radius: 4px; border-bottom-left-radius: 4px; border: 1px solid lightslategray;border-right:none; padding:4px 8px; width: 70px;height:34px;`;
  ...
  historyFilterAllButton.style = `${
    historyFilter === 'all'
      ? 'background-color: gold; color: rgb(31, 33, 35);'
      : 'background-color: rgb(31, 33, 35); color: lightslategray;'
  } font-size:0.8em; cursor: pointer;border-top-right-radius: 4px; border-bottom-right-radius: 4px; border: 1px solid lightslategray; padding:4px 8px; width: 70px;height:34px;`;
        ...
        () => {
          ...
          historyItems.forEach((historyItem) => {
            ...
            if (
              ...
              historyItemText.textContent.toLowerCase().includes(searchValue.toLowerCase())
            ) {
  ...
  historySearchInput.style =
    'background-color: #1f2123; color: lightslategray; font-size:0.8em; border-radius: 4px; border: 1px solid lightslategray; padding:4px 8px; width: 100%;margin-left:8px;';
  ...
  historySearchInput.addEventListener('input', (e) => {
    chrome.storage.local.get(['settings'], (result) => {
      ...
      const historyItems =
        result.settings.historyFilter === 'favorites'
      ...
      historyItems.forEach((item) => {
        ...
        if (
          itemText.textContent.toLowerCase().includes(value.toLowerCase()) ||
          value.trim().length === 0
        ) {
  ...
  historyItemTexts.forEach((historyItemText) => {
    ...
    if (!searchValue) {
      ...
      if (historyItemText.offsetHeight < historyItemText.scrollHeight) {
        ...
        const existingReadMoreButton = historyList?.querySelector(
          `div[id="read-more-history-item-${id}"]`,
        );
...
function historyModalActions() {
  ...
  actionBar.style =
    'display: flex; flex-direction: row; justify-content: space-between; align-items: end; width: 100%; margin: 8px 0;';
  const exportImportWrapper = document.createElement('div');
  exportImportWrapper.style =
    'display: flex; flex-direction: row; justify-content: start; align-items: end;';
  ...
  chrome.storage.local.get(['settings'], (result) => {
    exportHistoryButton.textContent = `Export ${
      result.settings.historyFilter === 'favorites' ? 'Favorites' : 'All'
    }`;
  });
  exportHistoryButton.addEventListener('click', () => {
    chrome.storage.local.get(['userInputValueHistory', 'settings'], (result) => {
      ...
      const historyItems =
        historyFilter === 'favorites'
          ? result.userInputValueHistory.filter((item) => item.isFavorite)
          : result.userInputValueHistory;
      const sortedHistoryItems = historyItems.sort((a, b) => b.timestamp - a.timestamp);
      const element = document.createElement('a');
      element.setAttribute(
        'href',
        `data:text/plain;charset=utf-8,${encodeURIComponent(JSON.stringify(sortedHistoryItems))}`,
      );
      ...
      element.setAttribute(
        'download',
        `superpower-chatgpt-prompt-history-${
  ...
  importHistoryButton.style =
    'font-size:0.7em; padding:4px 8px; margin-right:8px; width:130px;color:lightgray;';
      ...
      filePicker.addEventListener('change', (event) => {
        ...
        reader.onload = (e) => {
          ...
          importedHistory.forEach((importedItem) => {
            const existingItem = existingHistory.find(
              (item) => item.inputValue === importedItem.inputValue,
            );
            if (!existingItem) {
          ...
          chrome.storage.local.set({ userInputValueHistory: existingHistory }, () => {
            ...
            const allHistoryButton = document.querySelector(
              'button[id="history-filter-all-button"]',
            );
  ...
  clearHistoryButton.style =
    'font-size:0.7em; padding:4px 8px; margin-left:8px;width:130px;color:lightgray;';
...
function addUserPromptToHistory(inputValue) {
  ...
  chrome.storage.local.get(['userInputValueHistory', 'settings'], (result) => {
    ...
    chrome.storage.local.set({ userInputValueHistory }, () => {
      chrome.storage.local.set({
        userInputValueHistoryIndex: userInputValueHistory.length,
      });
    });
  });
}
...
function textAreaElementInputEventListener(event) {
  ...
  chrome.storage.local.set({ textInputValue: event.target.value.trim() }, () => {
    chrome.storage.local.get(['userInputValueHistory'], (result) => {
      ...
      if (existingInputValueIndex === -1) {
        chrome.storage.local.set({
          unsavedUserInput: event.target.value.trim(),
        });
      }
    });
  });
...
function textAreaElementKeydownEventListenerASync(event) {
  ...
  if (event.key === 'ArrowUp') {
    ...
    chrome.storage.local.get(
      ['userInputValueHistoryIndex', 'settings', 'userInputValueHistory'],
      (result) => {
        const { settings } = result;
  ...
  if (event.key === 'ArrowDown') {
    ...
    // if there is text in the field save that first
    chrome.storage.local.get(
      ['userInputValueHistoryIndex', 'settings', 'userInputValueHistory', 'unsavedUserInput'],
      (result) => {
        const { settings } = result;
        if (settings && !settings.promptHistory) return;
        let userInputValueHistoryIndex = result.userInputValueHistoryIndex || 0;
        const userInputValueHistory = result.userInputValueHistory || [];
        if (userInputValueHistory.length === 0) return;
        userInputValueHistoryIndex = Math.min(
          userInputValueHistoryIndex + 1,
          userInputValueHistory.length,
        );
        chrome.storage.local.set({ userInputValueHistoryIndex }, () => {
          const nextInputValue = userInputValueHistory[userInputValueHistoryIndex];
          if (nextInputValue) {
            textAreaElement.style.height = `${
              nextInputValue.inputValue.split('\\n').length * 24
            }px`;
  ...
  if (event.keyCode === 32) {
    chrome.storage.local.get(['customPrompts'], (res) => {
      ...
      if (lastWord.startsWith('@')) {
        const prompt = customPrompts.find(
          (p) => p.title.toLowerCase() === lastWord.substring(1).toLowerCase(),
        );
        if (prompt) {
          textAreaElement.value =
            textAreaValue.substring(0, textAreaValue.length - (lastWord.length + 1)) + prompt.text;
          textAreaElement.dispatchEvent(new Event('input', { bubbles: true }));
        }
      }
    });
...
function textAreaElementKeydownEventListenerSync(event) {
  ...
  if (event.key === 'Backspace') {
    ...
    if (
      ...
      cursorPosition - 1 > previousTriggerPosition &&
      textAreaElement.value.lastIndexOf(' ', cursorPosition) < previousTriggerPosition
    ) {
      const quickAccessMenuElement = document.querySelector('#quick-access-menu');
      if (!quickAccessMenuElement) {
