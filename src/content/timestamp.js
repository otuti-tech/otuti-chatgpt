  ...
  if (newChatButton.textContent.toLocaleLowerCase() !== 'new chat') {
    ...
    chrome.storage.local.get(['selectedConversations'], (result) => {
      ...
      exportAllButton.innerHTML = exportAllButton.innerHTML.replace(
        `Export ${selectedConversations.length} Selected`,
        'Export All',
      );
        ...
        chatButtons.forEach((button, index) => {
          ...
          if (!existingTimestamp) {
            ...
            checkboxWrapper.style =
              'position: absolute; top: 0px; left: 0px; z-index:10; display:none;cursor: pointer;width:40px;height: 100%;';
            ...
            checkbox.style =
              'position: absolute; top: 12px; left: 12px; z-index:11; cursor: pointer;';
            checkbox.checked = false;
            ...
            checkbox.addEventListener('click', (event) => {
              ...
              chrome.storage.local.get(
                ['lastSelectedConversation', 'selectedConversations'],
                (res) => {
                  ...
                  if (!event.target.checked) {
                    ...
                    chrome.storage.local.set(
                      ...
                      () => {
                        ...
                        updateButtonsAfterSelection(
                          selectedConversations,
                          newSelectedConversations,
                        );
                      },
                    );
                  }
                  ...
                  if (
                    ...
                    const newSelectedConversations = [...selectedConversations, conversation];
                    const conversationsOrder = Array.from(
                      conversationList.querySelectorAll('[id^=checkbox-wrapper-]'),
                    ).map((c) => c.id.split('checkbox-wrapper-')[1]?.slice(0, 5));
                    ...
                    if (lastSelectedConversation) {
                      ...
                      if (lastConversationIndex === -1 || newConversationIndex === -1) {
                        ...
                        if (
                          folderConatainingLastConversation?.id ===
                          folderConatainingNewConversation?.id
                        ) {
                          ...
                          const conversationsToSelect =
                            folderConatainingLastConversation.conversationIds
                              ?.slice(
