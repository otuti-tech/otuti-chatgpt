...
function createModal(title, subtitle, modalBodyContent, modalActionBarContent, allowFullscreen) {
  ...
  modal.style =
    'position:fixed;top:0;left:0;width:100vw;height:100vh;background-color:rgba(0,0,0,0.5);display:flex;justify-content:center;align-items:center;z-index:10000;';
  const modalWrapper = document.createElement('div');
  modalWrapper.id = `modal-wrapper-${title.toLowerCase().replaceAll(' ', '-')}`;
  modalWrapper.style = `width: ${window.innerWidth > 780 ? '50vw' : '100vw'}; height: ${
    window.innerWidth > 780 ? '70vh' : '80vh'
  }; background-color: #0b0d0e; border-radius: 8px; padding: 16px; display: flex; flex-direction: column; justify-content: space-between;box-shadow: rgb(0 0 0 / 72%) 0px 0px 20px 0px`;
  window.addEventListener('resize', () => {
    // eslint-disable-next-line no-nested-ternary
    modalWrapper.style.width =
      window.innerWidth > 960 ? '50vw' : window.innerWidth > 780 ? '70vw' : '100vw';
    // eslint-disable-next-line no-nested-ternary
    modalWrapper.style.height =
      window.innerWidth > 960 ? '70vh' : window.innerWidth > 780 ? '80vh' : '90vh';
  });
  ...
  modalCloseButton.style =
    'border-radius: 4px; font-size: 0.8em; border: 1px solid lightslategray; padding: 4px 8px;color: lightslategray;';
  ...
  modalBody.style = `display: flex; flex-direction: column; justify-content: space-between; border: solid 1px lightslategray; border-radius: 8px; height: 100%; overflow-y: ${
    allowFullscreen ? 'scroll' : 'hidden'
  };position: relative;`;
