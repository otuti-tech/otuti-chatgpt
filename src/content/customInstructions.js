...
function checkmarkIcon(placement, profileId) {
  ...
  checkmark.classList =
    'absolute inset-y-0 right-0 flex items-center pr-2 text-gray-800 dark:text-gray-100';
  checkmark.innerHTML =
  ...
  trash.addEventListener('click', (e) => {
    ...
    chrome.storage.local.get(['customInstructionProfiles'], (res) => {
      ...
      chrome.storage.local.set({ customInstructionProfiles: newCip }, () => {
        document
          .querySelectorAll(
            `[id^="custom-instructions-profile-dropdown-item-"][id$="-${profileId}"]`,
          )
...
function profileDropdown(customInstructionProfiles, placement) {
  ...
  dropdown.classList =
    'hidden absolute z-10 right-0 mt-1 overflow-auto rounded py-1 text-base ring-1 ring-opacity-5 focus:outline-none bg-white dark:bg-gray-800 dark:ring-white/20 dark:last:border-0 sm:text-sm -translate-x-1/4';
  dropdown.setAttribute('role', 'menu');
  dropdown.setAttribute('aria-orientation', 'vertical');
  dropdown.setAttribute(
    'aria-labelledby',
    `custom-instructions-profile-dropdown-button-${placement}`,
  );
  ...
  for (let i = 0; i < newCustomInstructionProfiles.length; i += 1) {
    ...
    dropdownOption.style =
      'white-space: nowrap; overflow: hidden; text-overflow: ellipsis;display:block;margin-right: 24px; text-align:left';
    ...
    dropdownItem.addEventListener('click', () => {
      const customInstructionsDialog = document.querySelector(
        '[role="dialog"][data-state="open"][tabindex="-1"]',
      );
      ...
      if (profileName === '+ Add new profile' && !customInstructionsDialog) {
        ...
        setTimeout(() => {
          ...
          const customInstructionsButton = [...menuButtons].find(
            (b) => b.textContent === 'Custom instructions',
          );
          customInstructionsButton.click();

          setTimeout(() => {
            const cusstomInstructionsProfileDropdown = document.querySelector(
              '#custom-instructions-profile-dropdown-list-settings',
            );
      ...
      if (customInstructionsDialog) {
        ...
        const textAreaFields = document.querySelectorAll(
          '[role="dialog"][data-state="open"][tabindex="-1"] textarea',
        );
      ...
      chrome.storage.local.get(['customInstructionProfiles'], (result) => {
        ...
        setTimeout(() => {
          ...
          chrome.storage.local.set({ customInstructionProfiles: newCip }, () => {
            const selectedProfileTitle = document.querySelector(
              `#custom-instructions-selected-profile-title-${placement}`,
            );
            selectedProfileTitle.textContent = profileName;
            // remove the old checkmark from the previous selected profile
            const oldCheckmark = document.querySelector(
              `[id^=custom-instructions-profile-dropdown-checkmark-${placement}-`,
            );
            if (oldCheckmark) oldCheckmark.remove();
            // add the new trash icon to the previous selected profile
            if (previousSelectedProfile) {
              const previousSelectedProfileDropdownItem = document.querySelector(
                `#custom-instructions-profile-dropdown-item-${placement}-${previousSelectedProfile?.id}`,
              );
            ...
            const oldTrash = dropdownItem.querySelector(
              `#custom-instructions-profile-dropdown-trash-${placement}-${profileId}`,
            );
            ...
            // hide the dropdown
            const customInstructionsProfileDropdown = document.querySelector(
              `#custom-instructions-profile-dropdown-list-${placement}`,
            );
  ...
  document.addEventListener('click', (e) => {
    ...
    if (
      cl?.contains('block') &&
      !e.target.closest(`#custom-instructions-profile-dropdown-button-${placement}`)
    ) {
      customInstructionsProfileDropdown.classList.replace('block', 'hidden');
    }
  });
...
function profileDropdownButton(customInstructionProfiles, placement) {
  ...
  button.className =
    'w-full relative cursor-pointer rounded-md border bg-white border-gray-300 pt-1 pl-3 pr-10 text-left focus:border-green-600 focus:outline-none focus:ring-1 focus:ring-green-600 dark:border-white/20 dark:bg-gray-800 sm:text-sm';
  button.type = 'button';
  ...
  span3.textContent =
    selectedProfile?.name || customInstructionProfiles[0]?.name || 'No saved profile';
  span2.appendChild(span3);
  const span4 = document.createElement('span');
  span4.className = 'pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2';
  button.appendChild(span4);
  span4.innerHTML =
    '<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4  text-gray-400" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polyline points="6 9 12 15 18 9"></polyline></svg>';
      ...
      if (mutation.type === 'childList') {
        setTimeout(() => {
          chrome.storage.local.get(['customInstructionProfiles'], (result) => {
            ...
            const existingProfileButtonWrapper = customInstructionsDialog.querySelector(
              '#custom-instructions-profile-button-wrapper-settings',
            );
            const textAreaFields = customInstructionsDialog.querySelectorAll('textarea');
            if (
              textAreaFields.length > 0 &&
              !existingProfileButtonWrapper &&
              customInstructionsDialog &&
              customInstructionsDialogHeader.textContent === 'Custom instructions'
            ) {
              ...
              profileButtonWrapper.appendChild(
                profileDropdown(newCustomInstructionProfiles, 'settings'),
              );
              profileButtonWrapper.appendChild(
                profileDropdownButton(newCustomInstructionProfiles, 'settings'),
              );
              ...
              nameLabel.className =
                'block text-xs text-gray-700 dark:text-gray-500 mb-2 text-gray-600';
              ...
              nameInput.classList =
                'w-full rounded p-2 mb-6 border dark:bg-gray-800 bg-white border-gray-100 focus:border-brand-green focus:ring-0 focus-visible:ring-0 bg-gray-50 outline-none focus-visible:outline-none';
              ...
              nameInput.addEventListener('input', () => {
                ...
                const curTextAreaFields = document.querySelectorAll(
                  '[role="dialog"][data-state="open"][tabindex="-1"] textarea',
                );
                ...
                if (
                  nameInput.value === '' ||
                  (nameInput.value === curSelectedProfileName &&
                    curAboutUserInput.value === selectedProfile?.aboutUser &&
                    curAboutModelInput.value === selectedProfile?.aboutModel)
                ) {
                ...
                chrome.storage.local.get(['customInstructionProfiles'], (res) => {
                  ...
                  if (!curSelectedProfile) {
                    const newCip = [
                      ...cip,
                      {
                        name: curNameInput.value,
                        aboutUser: curAboutUserInput.value,
                        aboutModel: curAboutModelInput.value,
                        isSelected: true,
