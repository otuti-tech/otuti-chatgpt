...
function generateSuggestions(conversationId, messageId, model, numSuggestions = 2) {
  ...
  return chrome.storage.sync
    ...
    .then((result) =>
      fetch(
        `https://chat.openai.com/backend-api/conversation/${conversationId}/experimental/generate_suggestions`,
        {
          method: 'POST',
          headers: {
            ...defaultHeaders,
...
function generateChat(
  ...
  return chrome.storage.local.get(['settings', 'enabledPluginIds']).then((res) =>
    chrome.storage.sync.get(['auth_token']).then((result) => {
      ...
      const eventSource = new SSE('/backend-api/conversation', {
        method: 'POST',
        headers: {
          ...defaultHeaders,
...
function getConversation(conversationId) {
  return chrome.storage.local
    ...
    .then((res) => {
      ...
      if (
        (typeof autoSync === 'undefined' || autoSync) &&
      ...
      return chrome.storage.sync.get(['auth_token']).then((result) =>
...
function getAccount() {
  return chrome.storage.sync
    .get(['auth_token'])
    .then((result) =>
      fetch('https://chat.openai.com/backend-api/accounts/check', {
        method: 'GET',
        headers: {
          ...defaultHeaders,
          Authorization: result.auth_token,
    ...
    .then((data) => {
      chrome.storage.local.get(['customInstructionProfiles'], (result) => {
        ...
        const newCustomInstructionProfiles = customInstructionProfiles.map((p) => {
          ...
          if (
            p.aboutModel === data.about_model_message &&
            p.aboutUser === data.about_user_message
          ) {
...
function getConversationLimit() {
  return chrome.storage.sync.get(['auth_token']).then((result) =>
    fetch('https://chat.openai.com/public-api/conversation_limit', {
      method: 'GET',
      headers: {
        ...defaultHeaders,
        Authorization: result.auth_token,
...
function messageFeedback(conversationId, messageId, rating, text = '') {
  ...
  return chrome.storage.sync.get(['auth_token']).then((result) =>
    fetch('https://chat.openai.com/backend-api/conversation/message_feedback', {
      method: 'POST',
      headers: {
        ...defaultHeaders,
        Authorization: result.auth_token,
...
function getSelectedConversations(forceRefresh = false) {
  return new Promise((resolve) => {
    chrome.storage.local.get(['selectedConversations'], (result) => {
      if (
        !forceRefresh &&
        result.selectedConversations &&
        result.selectedConversations.length > 0
      ) {
      ...
      chrome.storage.local
        ...
        .then((res) => {
          ...
          if (
            ...
            const initialLimit =
              autoSync && quickSync && quickSyncCount > 0 ? Math.min(100, quickSyncCount) : 100;
            getConversations(initialOffset, initialLimit).then(
              (convs) => {
                const { limit, offset, items } = convs;
                // eslint-disable-next-line no-nested-ternary
                const total =
                  autoSync && quickSync && quickSyncCount > 0
                    ? quickSyncCount
                    : convs.total
                    ? Math.min(convs.total, 10000)
                    : 10000; // sync last 10000 conversations
                  ...
                  Promise.all(promises).then(
                    ...
                    (err) => {
                      if (conversationsAreSynced) {
                        const visibleConversation = Object.values(conversations).filter(
...
function generateTitle(conversationId, messageId) {
  return chrome.storage.local.get(['settings']).then((res) => {
    ...
    return chrome.storage.sync.get(['auth_token']).then((result) =>
      fetch(`https://chat.openai.com/backend-api/conversation/gen_title/${conversationId}`, {
        method: 'POST',
        headers: {
          ...defaultHeaders,
          Authorization: result.auth_token,
