...
function createTemplateWordsModal(templateWords) {
  ...
  createModal(
    'Template words',
    'Please replace the template words',
    bodyContent,
    actionsBarContent,
    true,
  );
...
function templateWordsModalContent(templateWords) {
  ...
  templateWords.forEach((templateWord) => {
    ...
    templateWordLabel.style =
      'width: 100%;color:white;text-transform: capitalize;margin-bottom: 8px;';
    // remove the {{ curly }} braces
    templateWordLabel.innerHTML = templateWord.replace(/{{|}}/g, '');
    const templateWordInput = document.createElement('textarea');
    templateWordInput.style =
      'width: 100%; height: 100px; min-height: 100px; border-radius: 4px; border: 1px solid #565869; background-color: #1e1e2f; color: #eee; padding: 4px 8px; font-size: 14px;margin-right: 8px;';
...
function templateWordsModalActions(templateWords) {
  ...
  actionBar.style =
    'display: flex; flex-wrap:wrap;justify-content: end; align-items: center;width: 100%; font-size: 12px;';
  const submitButton = document.createElement('button');
  submitButton.style =
    'background-color: #565869; color: #eee; border: 1px solid #565869; border-radius: 4px; padding: 4px 8px; margin: 8px 0; width: 60px; height: 30px;';
