...
function promptChainListModalContent() {
  ...
  chrome.storage.local.get(['promptChains'], (result) => {
    ...
    if (allPromptChains.length === 0) {
      promptChainList.innerHTML = '<div style="font-size:1em;">You currently have no prompt chains saved. Click on Create New Prompt Chain button or create a prompt chain from any existing conversation using the button on the right side of the screen.</div>';
    } else {
...
function createPromptChainElement(promptChain) {
  ...
  promptChainEditButton.title = 'Edit Prompt Chain';
  ...
  promptChainDuplicateButton.title = 'Duplicate Prompt Chain';
  ...
  promptChainDeleteButton.title = 'Delete Prompt Chain';
...
function createPromptChainStep(promptChain, stepIndex, isNew = false) {
  ...
  if (!isNew) {
    ...
    runFromHereButton.title = 'Run From here';
  ...
  promptInput.placeholder = 'Enter prompt text. Tip: Use {{variable name}} to prompt user for input. To ask user to upload file start your prompt with {{files}} (GPT-4 only)';
...
function runPromptChain(promptChainSteps, initialStep = 0, newChat = true) {
  ...
  runningPromptChainStepCount.innerHTML = `Running prompt chain: ${runningPromptChainStepIndex + 1} / ${runningPromptChainSteps.length}`;
...
function insertNextChain(promptChainSteps, promptChainStepIndex) {
  ...
  if (runningPromptChainStepCount) runningPromptChainStepCount.innerHTML = `Running prompt chain: ${promptChainStepIndex + 1} / ${promptChainSteps.length}`;
