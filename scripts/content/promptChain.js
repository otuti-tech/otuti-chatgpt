...
function createPromptChainListModal() {
  ...
  createModal(
    'Prompt Chains',
    'You can find and run all of your existing prompt chains here',
    bodyContent,
    actionsBarContent,
  );
}
function promptChainListModalContent() {
  ...
  promptChainListText.style =
    'width:100%;height:100%;display:flex;align-items:center;justify-content:flex-start;overflow-y:scroll;padding:16px;flex-direction:column;position:relative;';
  chrome.storage.local.get(['promptChains'], (result) => {
    const allPromptChains = result.promptChains || [];
    if (allPromptChains.length === 0) {
      promptChainListText.innerHTML =
        '<div style="font-size:1em;">You currently have no prompt chains saved. You can create a prompt chain from any conversation. When you save a new prompt chain, it will show up here.</div>';
    } else {
      allPromptChains.forEach((promptChain, index) => {
        const promptChainElement = document.createElement('div');
        promptChainElement.style =
          'display:flex;align-items:center;justify-content:space-between;width:100%;padding:4px 8px; border: 1px solid #565869;border-radius:4px;margin-bottom:16px;color:#fff;cursor:pointer;';
        ...
        promptChainName.title = promptChain.steps
          .map((step, i) => `Step ${i + 1}:\n${step}`)
          .join('\n\n');
        ...
        promptChainActions.style =
          'display:flex;align-items:center;justify-content:flex-end;width:25%;';
        const promptChainRunButton = document.createElement('button');
        promptChainRunButton.classList =
          'btn flex justify-center gap-2 btn-dark border-0 md:border mr-1';
        promptChainRunButton.innerHTML =
          '<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>';
        ...
        promptChainEditButton.classList =
          'btn flex justify-center gap-2 btn-dark border-0 md:border mr-1';
        promptChainEditButton.innerHTML =
        ...
        promptChainDeleteButton.classList =
          'btn flex justify-center gap-2 btn-dark border-0 md:border';
        promptChainDeleteButton.innerHTML =
...
function createPromptChainStep(promptChain, index) {
  ...
  promptRow.style =
    'width:100%;display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;border-radius:4px;align-self:stretch;';
  ...
  promptInputActionWrapper.style =
    'display:flex;flex-direction:column;align-items:center;justify-content:space-between;width:70px;min-height:140px;max-height:140px; border: 1px solid #565869;border-right:none;border-top-left-radius:4px;border-bottom-left-radius:4px;';
  const deletePromptButton = document.createElement('button');
  deletePromptButton.style =
    'width: 100%; height: 70px; border: none;display:flex;align-items:center;justify-content:center; background-color: #2d2d3a; color: #eee; border-bottom: 1px solid #565869;border-top-left-radius:4px;';
  ...
  dragPromptButton.style =
    'width: 100%; height: 70px; display:flex;align-items:center;justify-content:center; border: none; background-color: #2d2d3a; color: #eee;border-bottom-left-radius:4px;cursor:grab;';
  ...
  promptInput.style =
    'width: 100%; height: 140px; min-height: 140px; border-top-right-radius: 4px;border-bottom-right-radius: 4px; border: 1px solid #565869; background-color: #2d2d3a; color: #eee; padding: 4px 8px; font-size: 14px; resize: none;';
...
function createNewPromptChainModal(promptChainName, promptChainSteps, chainIndex) {
  ...
  newPromptChainModal.style =
    'position:fixed;top:0px;left:0px;width:100%;height:100%;background-color:rgba(0,0,0,0.7);z-index:1000;display:flex;align-items:center;justify-content:center;z-index:10001;overflow-y: scroll; max-height: 100vh;';
  ...
  newPromptChainModalContent.style =
    'width:700px;height:70vh;max-height:70vh;background-color:#0b0d0e;border-radius:4px;padding:16px 0;display:flex;flex-direction:column;align-items:start;justify-content:start;border:solid 2px lightslategray;';
  newPromptChainModalContent.id = 'new-prompt-chain-modal-content';
  const modalTitle = document.createElement('div');
  modalTitle.style =
    'display:flex; align-items:center; justify-content:space-between;color:white;font-size:1.25rem; font-weight: bold; margin-bottom: 16px;padding: 0 16px;width:100%;';
  modalTitle.innerHTML = `${
    isNew ? '<span>Create prompt chain</span>' : '<span>Edit prompt chain</span>'
  } <button class="btn flex justify-center gap-2 btn-primary border-0 md:border" id="see-all-prompt-chains" style="margin:0 8px;">See all prompt chains</button>`;
  newPromptChainModalContent.appendChild(modalTitle);
  const modalSubtitle = document.createElement('div');
  modalSubtitle.style =
    'color:lightslategray;font-size:0.875rem; margin-bottom: 16px;padding: 0 16px;';
  modalSubtitle.textContent =
    'Prompt chains are a series of prompts that can be run automatically in sequence. You can create a new prompt chain from any existing conversation. You can add, remove, edit, or reorder prompts in the chain.';
  ...
  promptInputListWrapper.style =
    'width:100%;display:flex;flex-direction:column;align-items:start;justify-content:start;overflow-y: scroll;padding: 0 16px;scrollBehavior: smooth;';

  const promptInputNameInput = document.createElement('input');
  promptInputNameInput.id = 'prompt-chain-name-input';
  promptInputNameInput.style =
    'width: calc(100% - 40px); min-height: 40px; border-radius: 4px; border: 1px solid #565869; background-color: #2d2d3a; color: #eee; padding: 4px 8px; font-size: 14px; margin: 0 24px 16px 16px;';
  ...
  newPromptChainModalActionWrapper.style =
    'display:flex;align-items:center;justify-content:space-between;width:100%;margin-top:auto;padding:0 16px;';
  ...
  submitButton.addEventListener('click', () => {
    ...
    chrome.storage.local.get(['promptChains'], (res) => {
      ...
      if (isNew) {
        ...
        allPromptChains.unshift({
          title: promptChainName,
          steps: nonEmptySteps,
        });
      } else {
        allPromptChains[chainIndex] = {
          title: promptChainName,
          steps: nonEmptySteps,
        };
      }
...
function addPromptChainCreateButton() {
  ...
  promptChainCreateButton.style =
    'bottom: 11rem;right: 3rem;width: 2rem;height: 2rem;flex-wrap:wrap;border: solid 1px;';
  const plusIcon = document.createElement('div');
  plusIcon.classList = 'bg-gray-100 dark:bg-gray-700';
  plusIcon.style =
    'width: 1rem;height: 1rem;border: solid 1px;border-radius: 50%;border-color: currentColor;position: absolute;top: 0;right: 0;transform: translate(50%, -50%);';
  ...
  promptChainCreateButton.addEventListener('click', (e) => {
    ...
    const allUserMessageWrappers = document.querySelectorAll(
      '[id^="message-wrapper-"][data-role="user"]',
    );
