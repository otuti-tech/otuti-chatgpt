...
function replaceDeleteConversationButton() {
  ...
  deleteConversationsButton.classList =
  ...
  chrome.storage.local.get(['selectedConversations'], (result) => {
    ...
    deleteConversationsButton.innerHTML = `<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>Delete ${
      selectedConversations?.length === 0 ? 'All' : `${selectedConversations?.length} Selected`
    }`;
  });
    ...
    chrome.storage.sync.get(['conversationsOrder'], (syncResult) => {
      chrome.storage.local.get(
        ...
        (result) => {
          ...
          const { selectedConversations, conversations, conversationsAreSynced, settings } = result;
          const trashFolder = conversationsOrder?.find((folder) => folder.id === 'trash');
          const visibleConversations =
            conversationsAreSynced && conversations && settings.autoSync
              ? Object.values(conversations).filter(
                  (conversation) => !conversation.archived && !conversation.skipped,
                )
              : nav.querySelector('div.flex-col.flex-1').querySelector('div').querySelectorAll('a');
          ...
          if (e.target.textContent === 'Confirm Delete') {
            ...
            if (selectedConversations && selectedConversations?.length === 0) {
                ...
                (data) => {
                  if (data.success) {
                    ...
                    const conversationElements = Array.from(
                      conversationList.querySelectorAll('[id^=conversation-button-]'),
                    );
                    ...
                    conversationElements.forEach((conversationElement) => {
                      if (conversationsAreSynced && conversations && settings.autoSync) {
                        ...
                        if (
                          trashFolder &&
                          !trashFolder.conversationIds.includes(convId?.slice(0, 5))
                        ) {
                    ...
                    conversationList
                      .querySelectorAll('[id^=wrapper-folder-]')
              ...
              for (let i = 0; i < selectedConversationIds.length; i += 1) {
                promises.push(
                  deleteConversation(selectedConversationIds[i]).then(
                    (data) => {
                      if (data.success) {
                        successfullyDeletedConvIds.push(selectedConversationIds[i]);
                        const conversationElement = document.querySelector(
                          `#conversation-button-${selectedConversationIds[i]}`,
                        );
                        if (
                          conversationElement &&
                          conversationElement.classList.contains('selected')
                        ) {
                          showNewChatPage();
                        }
                        if (conversationsAreSynced && conversations && settings.autoSync) {
                          ...
                          const conversationElementIcon = conversationElement.querySelector('img');
                          conversationElementIcon.src = chrome.runtime.getURL('icons/trash.png');
                          const trashFolderContent =
                            document.querySelector('#folder-content-trash');
                          if (trashFolderContent) {
                            const emptyFolderElement =
                              trashFolderContent.querySelector('#empty-folder-trash');
                            if (emptyFolderElement) emptyFolderElement.remove();
                ...
                Promise.all(promises).then(() => {
                  ...
                  for (let i = 0; i < successfullyDeletedConvIds.length; i += 1) {
                    ...
                    let conversationOrderIndex = conversationsOrder.findIndex(
                      (id) => id === convId?.slice(0, 5),
                    );
                    if (conversationOrderIndex !== -1) {
                      conversationsOrder.splice(conversationOrderIndex, 1);
                    } else {
                      // if not found, look into folders
                      const conversationFolder = conversationsOrder.find(
                        (f) =>
                          f.id !== 'trash' &&
                          f.conversationIds &&
                          f.conversationIds.includes(convId?.slice(0, 5)),
                      );
                      if (conversationFolder) {
                        conversationOrderIndex = conversationFolder.conversationIds.findIndex(
                          (id) => id === convId?.slice(0, 5),
                        );
                        ...
                        if (conversationFolder.conversationIds.length === 0) {
                          const folderContent = document.querySelector(
                            `#folder-content-${conversationFolder.id}`,
                          );
                  ...
                  if (trashFolder) {
                    trashFolder.conversationIds = [
                      ...successfullyDeletedConvIds.map((cid) => cid?.slice(0, 5)),
                      ...trashFolder.conversationIds,
                    ];
            ...
            setTimeout(() => {
              chrome.storage.local.get(['selectedConversations'], (res) => {
                ...
                e.target.innerHTML = `<svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>Delete ${
                  selectedConvs.length === 0 ? 'All' : `${selectedConvs.length} Selected`
                }`;
                e.target.style.removeProperty('background-color');
                e.target.style.color = 'white';
              });
