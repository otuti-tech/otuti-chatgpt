...
const notSelectedClassList =
  'flex py-3 px-3 pr-3 w-full items-center gap-3 relative rounded-md hover:bg-[#2A2B32] cursor-pointer break-all hover:pr-20 group';
const selectedClassList =
  'flex py-3 px-3 pr-3 w-full items-center gap-3 relative rounded-md cursor-pointer break-all hover:pr-20 bg-gray-800 hover:bg-gray-800 group selected border-l border-gold';
...
function conversationActions(conversationId) {
  ...
  actionsWrapper.classList =
    'absolute flex right-1 z-10 text-gray-300 invisible group-hover:visible';
  ...
  editConversationNameButton.innerHTML =
    ...
    chrome.storage.sync.get(['name'], (syncResult) => {
      chrome.storage.local.get(['conversations'], (result) => {
        ...
        createShare(conversationId, currentNodeId).then((res) => {
          ...
          shareModalWrapper.innerHTML = shareModal(
  ...
  deleteConversationButton.innerHTML =
  ...
  deleteConversationButton.addEventListener('click', (e) => {
    ...
    const cancelButtons = document.querySelectorAll(
      `button[id^="cancel-"]:not(#cancel-${conversationId})`,
    );
        ...
        (data) => {
          if (data.success) {
            ...
            chrome.storage.sync.get(['conversationsOrder'], (res) => {
              ...
              let conversationOrderIndex = conversationsOrder.findIndex(
                (id) => id === conversation.id?.slice(0, 5),
              );
              if (conversationOrderIndex !== -1) {
                conversationsOrder.splice(conversationOrderIndex, 1);
              } else {
                // if not found, look into folders
                const conersationFolder = conversationsOrder.find(
                  (f) =>
                    f.id !== 'trash' && f.conversationIds.includes(conversation.id?.slice(0, 5)),
                );
                if (conersationFolder) {
                  conversationOrderIndex = conersationFolder.conversationIds.findIndex(
                    (id) => id === conversation.id?.slice(0, 5),
                  );
                  ...
                  if (conersationFolder.conversationIds.length === 0) {
                    const folderContent = document.querySelector(
                      `#folder-content-${conersationFolder.id}`,
                    );
              ...
              if (!trashFolder.conversationIds.includes(conversation.id?.slice(0, 5))) {
                conversationsOrder
                  ...
                  .conversationIds.unshift(conversation.id?.slice(0, 5));
              }
...
function addCheckboxToConversationElement(conversationElement, conversation) {
  chrome.storage.local.get(['selectedConversations'], (result) => {
    ...
    checkboxWrapper.style =
      'position: absolute; top: 0px; left: 0px; z-index:10; display:none;cursor: pointer;width:40px;height: 100%;border:none;border-radius:6px;';
    ...
    checkbox.addEventListener('click', (event) => {
      ...
      chrome.storage.local.get(
        ['lastSelectedConversation', 'conversations', 'selectedConversations'],
        (res) => {
          ...
          if (
            ...
            ((!event.shiftKey && !shiftKeyPressed) || selectedConversations.length === 0)
          ) {
          ...
          if (
            ...
            chrome.storage.sync.get(['conversationsOrder'], (syncResult) => {
              ...
              if (lastSelectedConversation) {
                ...
                let lastConversationIndex = conversationsOrder.findIndex(
                  (c) => c === lastSelectedConversation.id?.slice(0, 5),
                );
                let newConversationIndex = conversationsOrder.findIndex(
                  (c) => c === conversation.id?.slice(0, 5),
                );

                if (lastConversationIndex === -1 || newConversationIndex === -1) {
                  const folderConatainingLastConversation = conversationsOrder.find((f) =>
                    f.conversationIds?.find(
                      (cid) => cid === lastSelectedConversation.id?.slice(0, 5),
                    ),
                  );

                  const folderConatainingNewConversation = conversationsOrder.find((f) =>
                    f.conversationIds?.find((cid) => cid === conversation.id?.slice(0, 5)),
                  );

                  if (
                    folderConatainingLastConversation?.id === folderConatainingNewConversation?.id
                  ) {
                    lastConversationIndex =
                      folderConatainingLastConversation?.conversationIds?.findIndex(
                        (cid) => cid === lastSelectedConversation.id?.slice(0, 5),
                      );
                    newConversationIndex =
                      folderConatainingNewConversation?.conversationIds?.findIndex(
                        (cid) => cid === conversation.id?.slice(0, 5),
                      );
                    const conversationsToSelect = folderConatainingLastConversation.conversationIds
                      ?.slice(
                        Math.min(lastConversationIndex, newConversationIndex) + 1,
                        Math.max(lastConversationIndex, newConversationIndex),
                      )
                  ...
                  const conversationsToSelect = conversationsOrder
                    ?.slice(
                      Math.min(lastConversationIndex, newConversationIndex) + 1,
                      Math.max(lastConversationIndex, newConversationIndex),
                    )
...
function updateButtonsAfterSelection(previousSelectedConversations, newSelectedConversations) {
  const previousText =
    previousSelectedConversations.length === 0
      ? 'All'
      : `${previousSelectedConversations.length} Selected`;
  const newText =
    newSelectedConversations.length === 0 ? 'All' : `${newSelectedConversations.length} Selected`;
  ...
  if (deleteConversationsButton) {
    deleteConversationsButton.innerHTML = deleteConversationsButton.innerHTML.replace(
      `Delete ${previousText}`,
      `Delete ${newText}`,
    );
  }
