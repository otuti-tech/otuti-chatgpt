...
async function addConversationToStorage(conv) {
  await getConversation(conv.id).then(
    (conversation) => {
      ...
      if (Object.keys(localConversations).length > 0) {
        chrome.storage.sync.get(['conversationsOrder'], (result) => {
          ...
          chrome.storage.sync.set({
            conversationsOrder: conversationsOrder.includes(conv.id?.slice(0, 5))
              ? conversationsOrder
              : [conv.id?.slice(0, 5), ...conversationsOrder],
          });
        });
      }
    },
...
function updateOrCreateConversation(
  conversationId,
  message,
  parentId,
  settings,
      ...
      return chrome.storage.local
        ...
        .then(() => {
          ...
          if (
            ...
            mapping.length < 5 &&
            mapping.filter((m) => m.message?.author.role === 'assistant').length === 1
          ) {
            ...
            if (settings.saveHistory) {
              ...
              generateTitleForConversation(
                ...
                message.id,
                systemMessage?.message?.metadata?.user_context_message_data,
              );
            }
          } else if (settings.conversationTimestamp) {
            // === updated
...
function addProgressBar() {
  chrome.storage.local.get(['settings'], (result) => {
    ...
    tooltip.style =
      'position: absolute; width: 250px; border: solid 1px #8e8ea0; bottom: 20px; right: 4px; background-color: #343541; display:none; margin:0;';
    tooltip.id = 'sync-tooltip';
    tooltip.innerText =
      'You conversations are being backed up in your computer for a faster experience! This can take a while.';
    ...
    refreshButton.innerHTML =
      '<svg class="animate-spin" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="#00aaff" d="M468.9 32.11c13.87 0 27.18 10.77 27.18 27.04v145.9c0 10.59-8.584 19.17-19.17 19.17h-145.7c-16.28 0-27.06-13.32-27.06-27.2c0-6.634 2.461-13.4 7.96-18.9l45.12-45.14c-28.22-23.14-63.85-36.64-101.3-36.64c-88.09 0-159.8 71.69-159.8 159.8S167.8 415.9 255.9 415.9c73.14 0 89.44-38.31 115.1-38.31c18.48 0 31.97 15.04 31.97 31.96c0 35.04-81.59 70.41-147 70.41c-123.4 0-223.9-100.5-223.9-223.9S132.6 32.44 256 32.44c54.6 0 106.2 20.39 146.4 55.26l47.6-47.63C455.5 34.57 462.3 32.11 468.9 32.11z"/></svg>';
...
function checkConversationAreSynced(localConvs, remoteConvs) {
  return (
    Object.values(localConvs).filter(
      (conv) => !conv.archived && (typeof conv.saveHistory === 'undefined' || conv.saveHistory),
    ).length === remoteConvs.length
  );
}
// eslint-disable-next-line no-unused-vars
...
function inSyncConversations(localConvs, remoteConvs) {
  return Object.keys(localConvs).filter(
    (id) =>
      !localConvs[id].archived &&
      !localConvs[id].shouldRefresh &&
      (localConvs[id].current_node || localConvs[id].skipped) &&
      remoteConvs.find((rc) => rc.id === id)?.update_time &&
      (localConvs[id].update_time ===
        new Date(remoteConvs.find((rc) => rc.id === id).update_time).getTime() / 1000 ||
        localConvs[id].skipped) &&
...
function initializeAutoSave(skipInputFormReload = false, forceRefreshIds = []) {
  ...
  getAllConversations(forceRefresh).then(
    (remoteConversations) => {
      chrome.storage.sync.get(['conversationsOrder'], (res) => {
        ...
        chrome.storage.local.get(['conversations', 'account', 'settings'], (result) => {
          ...
          const oldConversationsOrder =
            conversationsOrder && conversationsOrder?.findIndex((f) => f && f.id === 'trash') !== -1
              ? conversationsOrder
              : [
                  {
                    id: 'trash',
          ...
          chrome.storage.local.set(
            ...
            () => {
              ...
              const oldConversationsOrderHasLongIds =
                oldConversationsOrder.findIndex((conv) => {
                  if (typeof conv === 'string') {
                    return conv.length > 5;
                  }
                  return (
                    conv.id.length > 5 ||
                    conv.conversationIds.findIndex((id) => id.length > 5) !== -1
                  );
                }) !== -1;
              ...
              const newConversationsOrder = oldConversationsOrderHasLongIds
                ? oldConversationsOrder.map((conv) => {
                    ...
                    return {
                      ...conv,
                      id: conv.id?.slice(0, 5),
                      conversationIds: conv.conversationIds.map((id) => id?.slice(0, 5)),
                    };
                  })
              ...
              chrome.storage.sync.set(
                ...
                async () => {
                  ...
                  const visibleAndNotSkippedLocalConvIds = Object.keys(localConversations).filter(
                    (id) => !localConversations[id].archived && !localConversations[id].skipped,
                  );
                  const visibleAndRefreshedLocalConvIds = Object.keys(localConversations).filter(
                    (id) =>
                      !localConversations[id].archived && !localConversations[id].shouldRefresh,
                  );

                  for (let i = 0; i < localConvIds.length; i += 1) {
                    const remoteConv =
                      remoteConversations.find((conv) => conv.id === localConvIds[i]) ||
                      localConversations[localConvIds[i]];
                    localConversations[localConvIds[i]].title = remoteConv.title;
                    // eslint-disable-next-line prefer-destructuring
                    if (localConversations[localConvIds[i]].update_time === 'force_copy') {
                      localConversations[localConvIds[i]].update_time =
                        new Date(remoteConv.update_time).getTime() / 1000;
                    }
                    // delete conversation key(legacy)
                    ...
                    if (
                      typeof localConversations[localConvIds[i]].writingStyleCode === 'undefined'
                    ) {
                    ...
                    if (
                      localConversations[localConvIds[i]].id &&
                      localConversations[localConvIds[i]].saveHistory &&
                      !remoteConvIds.includes(localConversations[localConvIds[i]].id)
                    ) {
                      localConversations[localConvIds[i]].archived = true;
                      // check conversations
                      if (
                        newConversationsOrder.indexOf(
                          localConversations[localConvIds[i]].id?.slice(0, 5),
                        ) !== -1
                      ) {
                        newConversationsOrder.splice(
                          newConversationsOrder.indexOf(
                            localConversations[localConvIds[i]].id?.slice(0, 5),
                          ),
                          1,
                        ...
                        newConversationsOrder.forEach((folder) => {
                          if (
                            ...
                            folder.id !== 'trash' &&
                            folder.conversationIds.indexOf(
                              localConversations[localConvIds[i]].id?.slice(0, 5),
                            ) !== -1
                          ) {
                            folder.conversationIds.splice(
                              folder.conversationIds.indexOf(
                                localConversations[localConvIds[i]].id?.slice(0, 5),
                              ),
                              1,
                      ...
                      const trashFolder = newConversationsOrder.find(
                        (folder) => folder?.id === 'trash',
                      );
                      ...
                      if (
                        !settings.quickSync &&
                        !trashFolder?.conversationIds.includes(
                          localConversations[localConvIds[i]].id?.slice(0, 5),
                        )
                      ) {
                        newConversationsOrder
                          .find((folder) => folder?.id === 'trash')
                  ...
                  const allVisibleConversationsOrderIds = newConversationsOrder
                    .filter((conv) => conv.id !== 'trash')
                    .map((conv) => (typeof conv === 'object' ? conv.conversationIds : conv))
                    .flat();

                  if (
                    remoteConvIds.length > 0 &&
                    remoteConvIds.length -
                      inSyncConversations(localConversations, remoteConversations).length >
                      3
                  ) {
                  ...
                  for (let i = 0; i < remoteConvIds.length; i += 1) {
                    ...
                    if (
                      localConvIds.includes(remoteConvIds[i]) &&
                    ...
                    if (!allVisibleConversationsOrderIds.includes(remoteConvIds[i]?.slice(0, 5))) {
                      if (!conversationsOrder || conversationsOrder.length === 0) {
                        // if conversationsOrder does not exist, add to the end of it right before trash folder (last element -1)
                        newConversationsOrder.splice(
                          newConversationsOrder.length - 1,
                          0,
                          remoteConvIds[i]?.slice(0, 5),
                        );
                      } else {
                        // if conversationsOrder exists, insert it at index i in newConversationsOrder
                        newConversationsOrder.splice(i, 0, remoteConvIds[i]?.slice(0, 5));
                      }
                    }
                    if (
                      forceRefreshIds.includes(remoteConvIds[i]) ||
                      !visibleAndNotSkippedLocalConvIds.includes(remoteConvIds[i]) ||
                      ...
                      (remoteConversations[i].current_node &&
                        !localConversations[remoteConvIds[i]].current_node) ||
                      (remoteConversations[i].update_time &&
                        localConversations[remoteConvIds[i]].update_time !==
                          new Date(remoteConversations[i].update_time).getTime() / 1000)
                  ...
                  newConversationsOrder.forEach((folder, index) => {
                    if (typeof folder === 'string') {
                      if (
                        newConversationsOrder.indexOf(folder) !==
                        newConversationsOrder.lastIndexOf(folder)
                      ) {
                    ...
                    if (typeof folder === 'object') {
                      ...
                      for (let i = 0; i < folder.conversationIds.length; i += 1) {
                        if (newConversationsOrder.indexOf(folder.conversationIds[i]) !== -1) {
                          newConversationsOrder.splice(
                            newConversationsOrder.indexOf(folder.conversationIds[i]),
                            1,
                          );
                        }
                      }
                  ...
                  const conversationsAreSynced = checkConversationAreSynced(
                    localConversations,
                    remoteConversations,
                  );
                        ...
                        chrome.storage.sync.set(
                          ...
                          () => {
                            ...
                            if (refreshButton) {
                              ...
                              refreshButton.innerHTML =
                                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="gold" d="M468.9 32.11c13.87 0 27.18 10.77 27.18 27.04v145.9c0 10.59-8.584 19.17-19.17 19.17h-145.7c-16.28 0-27.06-13.32-27.06-27.2c0-6.634 2.461-13.4 7.96-18.9l45.12-45.14c-28.22-23.14-63.85-36.64-101.3-36.64c-88.09 0-159.8 71.69-159.8 159.8S167.8 415.9 255.9 415.9c73.14 0 89.44-38.31 115.1-38.31c18.48 0 31.97 15.04 31.97 31.96c0 35.04-81.59 70.41-147 70.41c-123.4 0-223.9-100.5-223.9-223.9S132.6 32.44 256 32.44c54.6 0 106.2 20.39 146.4 55.26l47.6-47.63C455.5 34.57 462.3 32.11 468.9 32.11z"/></svg>';
