...
function updateQuickAccessMenuItems() {
  ...
  menuItems.forEach((item) => {
    ...
    if (itemText.toLowerCase().includes(triggerWord.toLowerCase())) {
      document.querySelector('#quick-access-menu').style.display = 'flex';
  ...
  if (!foundFirstVisibleItem && triggerWord.length > 0) {
    document.querySelector('#quick-access-menu').style.display = 'none';
  } else {
    document.querySelector('#quick-access-menu').style.display = 'flex';
  }
}
function quickAccessMenu(trigger) {
  chrome.storage.local.get(['account', 'chatgptAccountId'], (r) => {
    ...
    if (trigger !== '@' || (isPaid && (isGPT4 || isGizmo))) {
      ...
      menu.classList = 'absolute flex-col gap-2 bg-token-main-surface-primary border border-token-border-light rounded-xl shadow-xs';
      menu.style = 'height: 300px; top:-316px; left:0; width:100%; z-index: 10000;display:none;';
