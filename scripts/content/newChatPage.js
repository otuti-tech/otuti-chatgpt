...
/* global createSwitch, gptSVG, getUserSystemMessage, setUserSystemMessage, profileDropdown, profileDropdownButton, handleQueryParams, updateOutOfDateConversation, arkoseWasInitialized, notSelectedClassList, runningPromptChainSteps:true, runningPromptChainStepIndex:true, replaceTextAreaElement, initializeNavbar, replacePageContent, languageList, writingStyleList, toneList, getExamplePrompts, getGizmoUserActionSettings, removeTextInputExtras, registerWebsocket, getChatGPTAccountIdFromCookie, areSameArrays, dalleModes, addDalleModeEventListeners, stopConversationWS, stopAllAudios */
  ...
  chrome.storage.local.get(['account', 'chatgptAccountId', 'settings', 'models', 'unofficialModels', 'customModels'], (result) => {
    ...
    if (!isPaid || !arkoseEnabled || initializedArkose) {
      ...
      if (showExamplePrompts) {
        if (gizmoResource) {
          const isDallE = gizmoResource?.gizmo?.id === 'g-2fkFE8rbu';
          if (isDallE) {
...
function customInstructionSettingsElement() {
  ...
  getUserSystemMessage().then((systemMessage) => {
    ...
    chrome.storage.local.get(['customInstructionProfiles'], (result) => {
      ...
      if (!selectedProfile || selectedProfile.aboutUser.replace(/[^a-zA-Z]/g, '') !== (systemMessage?.about_user_message)?.toString()?.replace(/[^a-zA-Z]/g, '') || selectedProfile?.aboutModel?.replace(/[^a-zA-Z]/g, '') !== (systemMessage?.about_model_message)?.toString()?.replace(/[^a-zA-Z]/g, '') || !areSameArrays(selectedProfile?.disabledTools, systemMessage?.disabled_tools)) {
        newCustomInstructionProfiles = customInstructionProfiles.map((p) => {
          if (p.aboutModel.replace(/[^a-zA-Z]/g, '') === (systemMessage?.about_model_message)?.toString()?.replace(/[^a-zA-Z]/g, '') && p.aboutUser.replace(/[^a-zA-Z]/g, '') === (systemMessage?.about_user_message)?.toString()?.replace(/[^a-zA-Z]/g, '') && areSameArrays(p.disabledTools, systemMessage?.disabled_tools)) {
            return { ...p, disabledTools: systemMessage.disabled_tools, isSelected: true };
          }
          if (p.isSelected) {
            return { ...p, disabledTools: systemMessage.disabled_tools, isSelected: false };
          }
          return { ...p, disabledTools: systemMessage.disabled_tools };
        });
