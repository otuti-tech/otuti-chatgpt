/* global markdown, katex, texmath, markdownitSup, hljs, getDownloadUrlFromFileId, addMessagePluginToggleButtonsEventListeners, formatTime, openDalleCarousel, addDalleCarouselEventListeners, toast, downloadFileFrmoUrl, formatDateDalle */
...
function rowAssistant(conversation, nodes, childIndex, childCount, models, settings, gizmoData, isLoading = false) {
  ...
  const avatar = gizmoData ? gizmoData?.resource?.gizmo?.display?.profile_picture_url : '';
  let renderedNodes = '';
  ...
  class="group w-full text-token-text-primary border-b border-black/10 dark:border-gray-900/50 ${pinned ? 'border-l-pinned bg-pinned dark:bg-pinned' : 'bg-gray-50 dark:bg-[#444654]'}">
  ...
  <button id="message-pin-button-${id}" title="pin/unpin message" class="${pinned ? 'visible' : 'invisible group-hover:visible'}" style="background-color: transparent; border: none; cursor: pointer;width: 18px; position: absolute; top: -8px; right: 6px;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path fill="${pinned ? 'gold' : '#aaa'}" d="M48 0H336C362.5 0 384 21.49 384 48V487.7C384 501.1 373.1 512 359.7 512C354.7 512 349.8 510.5 345.7 507.6L192 400L38.28 507.6C34.19 510.5 29.32 512 24.33 512C10.89 512 0 501.1 0 487.7V48C0 21.49 21.49 0 48 0z"/></svg></button>
    ...
    ${gizmoData ? `<div class="relative flex h-9 w-9"><img src="${avatar}" class="h-full w-full bg-token-surface-secondary dark:bg-token-surface-tertiary rounded-full" alt="GPT" width="80" height="80">
    ...
    <div class="relative flex flex-col gap-1 md:gap-3 agent-turn" style="width:calc(100% - 80px);">
      <div class="flex flex-grow flex-col gap-3 max-w-full">
        ...
        <div id="result-action-wrapper-${id}"
          ...
          ${showWordCount ? `<div id="result-counter-${id}">${nodeCharCounts} chars / ${nodeWordCounts} words</div>` : ''}
          <button id="result-copy-button-${id}"
            class="btn flex justify-center gap-2 btn-neutral border-0 md:border" style="border-radius: 4px; border: 1px solid lightslategray; position: absolute; right: 0px; width: 64px; font-size:11px;padding:6px 12px;">Copy</button><div id="copy-result-menu-${id}" style="font-size: 10px; position: absolute; right: 0px; bottom: 37px;display:none;"><button id="result-markdown-copy-button-${id}"
...
function addAssistantNode(node, continueGenerating = false, existingInnerHTML = '') {
  ...
  if (existingMessageText) {
    const { assistantMessageHTML } = assistantContentGenerator(node);
...
function assistantRenderer(assistantNode) {
  ...
  const { assistantMessageHTML, wordCount, charCount } = assistantContentGenerator(assistantNode);
...
function assistantContentGenerator(assistantNode) {
  ...
  const wordCount = el.innerText.split(/[ /]/).length;
