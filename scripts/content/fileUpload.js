  ...
  for (let i = 0; i < files.length; i += 1) {
    ...
    reader.onload = () => {
      ...
      const fileElementHTML = isImage
        ? `<div id="file-element-${i + existingFileElements.length}" class="group relative inline-block text-sm text-black/70 dark:text-white/90"><div class="relative overflow-hidden rounded-xl border border-gray-200 dark:border-gray-600"><div class="h-14 w-14"><button type="button" id="full-size-file-button-${i + existingFileElements.length}" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-:r2c:" data-state="closed" class="h-full w-full"><span class="flex items-center h-full w-full justify-center bg-gray-500 dark:bg-gray-700 bg-cover bg-center text-white" style="background-image: url(&quot;${dataUrl}&quot;);"></span></button></div></div><button id="remove-file-button-${i + existingFileElements.length}" class="absolute right-1 top-1 -translate-y-1/2 translate-x-1/2 rounded-full border border-white bg-gray-500 p-0.5 text-white transition-colors hover:bg-black hover:opacity-100 group-hover:opacity-100 md:opacity-0"><span class="" data-state="closed"><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="icon-sm" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></span></button><div id="file-upload-spinner-${i}" class="absolute inset-0 flex items-center justify-center bg-black/50 text-white"><svg x="0" y="0" viewbox="0 0 40 40" class="spinner"><circle fill="transparent" stroke="#ffffff50" stroke-width="4" stroke-linecap="round" stroke-dasharray="125.6" cx="20" cy="20" r="18"></circle></svg></div></div>`
        : `<div id="file-element-${i + existingFileElements.length}" class="group relative inline-block text-sm text-black/70 dark:text-white/90"><div class="relative overflow-hidden rounded-xl border border-gray-200 dark:border-gray-600"><div class="p-2 dark:bg-gray-600 w-60"><div class="flex flex-row items-center gap-2"><div class="relative h-10 w-10 shrink-0 overflow-hidden rounded-md">${getThumbnail(fileType)}</div><div class="overflow-hidden"><div class="truncate font-medium">${file.name}</div><div class="truncate text-gray-300">${fileType}</div></div></div></div></div><button id="remove-file-button-${i + existingFileElements.length}" class="absolute right-1 top-1 -translate-y-1/2 translate-x-1/2 rounded-full border border-white bg-gray-500 p-0.5 text-white transition-colors hover:bg-black hover:opacity-100 group-hover:opacity-100 md:opacity-0"><span class="" data-state="closed"><svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="icon-sm" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></span></button><div id="file-upload-spinner-${i}" class="absolute inset-0 flex items-center justify-center bg-black/50 text-white"><svg x="0" y="0" viewbox="0 0 40 40" class="spinner"><circle fill="transparent" stroke="#ffffff50" stroke-width="4" stroke-linecap="round" stroke-dasharray="125.6" cx="20" cy="20" r="18"></circle></svg></div></div>`;
      ...
      const useCase = fileType === 'Image' ? 'multimodal' : fileType === 'PDF' ? 'my_files' : 'ace_upload';
        ...
        if (result.status === 'success') {
          uploadFileAPI(result.file_id, result.upload_url, file).then(async () => {
            uploadedAPI(result.file_id).then(() => {
              ...
              // remove spinner
              const spinner = document.querySelector(`#file-upload-spinner-${i}`);
