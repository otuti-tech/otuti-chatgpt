...
function removeUploadFileButton() {
  ...
  // remove all upload file buttons
  const uploadFileButtonWrappers = document.querySelectorAll('#upload-file-button-wrapper');
  uploadFileButtonWrappers.forEach((wrapper) => wrapper.remove());
  ...
  if (paperclipIcon) paperclipIcon.parentElement?.parentElement?.parentElement?.parentElement?.remove();

  // area-label='Attach files'
  const uploadButton = document.querySelector('button[aria-label="Anexar arquivos"]');
...
function addPageDragAndDropEventListener() {
  ...
  presentation.addEventListener('dragenter', (e) => {
    ...
    if (presentation.lastChild.textContent.includes('Anexar')) {
  ...
  presentation.addEventListener('dragleave', (e) => {
    ...
    if (presentation.lastChild.innerText.includes('Anexar')) {
      presentation.lastChild.remove();
    }
  });
...
function fileChangeHandler(files) {
  ...
  if (files.length + existingFileElements.length > uploadLimit) {
    toast(`Limite de arquivos simultâneos ultrapassado. O limite de envios é ${uploadLimit}`, 'error');
    return;
  }
      ...
      createFileInServer(file, useCase).then((result) => {
        ...
        if (result.status === 'success') {
          uploadFileAPI(result.file_id, result.upload_url, file).then(async () => {
            ...
            toast(`Indisponível para Upload ${file.name}`, 'error');
            // remove file element
