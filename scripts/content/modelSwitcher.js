...
/* global addUploadFileButton, arkoseTrigger, curImageAssets:true, curFileAttachments:true, registerWebsocket */
...
function createModelListDropDown(models, selectedModel, idPrefix, customModels = [], forceDark = false) {
  return `${models.filter((m) => !m.slug.includes('plugins')).map((model) => `<li class="text-gray-900 group relative cursor-pointer select-none border-b py-1 pl-3 pr-12 last:border-0 border-token-border-light hover:bg-token-main-surface-secondary" id="${idPrefix}-model-switcher-option-${model.slug}" role="option" tabindex="-1">
