...
function rowUser(conversation, node, childIndex, childCount, name, avatar, settings) {
  ...
  class="group w-full text-token-text-primary border-b border-black/10 dark:border-gray-900/50 ${pinned ? 'border-l-pinned bg-pinned dark:bg-pinned' : 'bg-token-main-surface-primary'}">
  ...
  <button id="message-pin-button-${id}" title="pin/unpin message" class="${pinned ? 'visible' : 'invisible group-hover:visible'}" style="background-color: transparent; border: none; cursor: pointer;min-width: 18px;position: absolute; top: -8px; right: 6px;z-index:0;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path fill="${pinned ? 'gold' : '#aaa'}" d="M48 0H336C362.5 0 384 21.49 384 48V487.7C384 501.1 373.1 512 359.7 512C354.7 512 349.8 510.5 345.7 507.6L192 400L38.28 507.6C34.19 510.5 29.32 512 24.33 512C10.89 512 0 501.1 0 487.7V48C0 21.49 21.49 0 48 0z"/></svg></button>
    ...
    <div class="absolute left-0 flex" style="bottom:-12px;left:16px;">
      ${languageName ? `<div id="language-code-${id}" title="You changed the response language here. This prompt includes a hidden language instructions" class="h-6 p-2 mr-1 flex items-center justify-center rounded-md border text-xs text-token-text-tertiary border-gray-500 dark:border-gray-400 bg-token-main-surface-secondary">Language: &nbsp<b>${languageName}</b></div>` : ''}
      ${toneName ? `<div id="tone-code-${id}" title="You changed the response tone here. This prompt includes a hidden tone instructions" class="h-6 p-2 mr-1 flex items-center justify-center rounded-md border text-xs text-token-text-tertiary border-gray-500 dark:border-gray-400 bg-token-main-surface-secondary">Tone: &nbsp<b>${toneName}</b></div>` : ''}
      ${writingStyleName ? `<div id="writing-style-code-${id}" title="You changed the response writing style here. This prompt includes a hidden writing style instructions" class="h-6 p-2 mr-1 flex items-center justify-center rounded-md border text-xs text-token-text-tertiary border-gray-500 dark:border-gray-400 bg-token-main-surface-secondary">Writing Style: &nbsp<b>${writingStyleName}</b></div>` : ''}
    </div>
  </div>
</div>
`;
