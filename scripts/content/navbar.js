...
function addNavbar(conversation = null) {
  ...
  navWrapper.classList = 'w-full z-1000 bg-transparent transition-all relative top-0';
  navWrapper.style = 'height: 56px;';
  const navbar = document.createElement('div');
  navbar.id = 'gptx-navbar';
  navbar.classList = 'w-full flex items-center justify-between border-b h-14 border-black/10 bg-gray-50 px-3 py-1 text-gray-500 dark:border-gray-900/50 dark:bg-gray-700 dark:text-gray-300 shadow-md';
  ...
  chrome.storage.local.get(['settings', 'models', 'installedPlugins', 'enabledPluginIds', 'unofficialModels', 'customModels'], (result) => {
    ...
    const {
      autoSync, selectedModel, selectedLanguage, selectedTone, selectedWritingStyle, autoHideTopNav,
    } = settings;
    if (autoHideTopNav) {
    ...
    chrome.storage.local.set({
      ...
      if (models.map((m) => m.slug).find((m) => m.includes('plugins'))) {
        ...
        if (newSelectedModel.slug.includes('plugins')) {
