...
function addNavbar() {
  ...
  navbar.className =
    'w-full flex items-center justify-between border-b h-14 border-black/10 bg-gray-50 px-3 py-1 text-gray-500 dark:border-gray-900/50 dark:bg-gray-700 dark:text-gray-300 shadow-md';
  ...
  chrome.storage.local.get(
    ...
    (result) => {
      ...
      if (models.map((m) => m.slug).find((m) => m.includes('plugins'))) {
        ...
        pluginsDropdownWrapper.style =
          'position:relative;width:200px;margin-left:8px;z-index:1000;display:none';
      ...
      writingStyleSelectorWrapper.innerHTML = dropdown(
        'Writing-Style',
        writingStyleList,
        selectedWritingStyle,
        'right',
