  ...
  chrome.storage.local.get(['account'], (r) => {
    ...
    gizmos?.forEach((gizmo) => {
      ...
      <div id="gizmo-card-${gizmo.id}" class="relative flex flex-col w-full h-full justify-start items-start gap-2 p-4 cursor-pointer bg-token-main-surface-secondary hover:bg-token-main-surface-tertiary hover:shadow-xl rounded-xl">
        ...
        <div style="min-height:22px;" class="mt-1 flex items-center gap-1 text-token-text-tertiary">${categories.slice(0, 2).map((category) => `<div id="category-tag-${category.id}" style='font-size:11px;' class="border rounded-full border-token-border-light hover:border-green-500 hover:cursor-pointer hover:text-green-500 px-2">${category.title}</div>`).join('')}</div>

        <div class="flex items-center gap-1 text-token-text-tertiary"><div class="text-sm text-token-text-tertiary" style="white-space: break-spaces; overflow-wrap: break-word;display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden;">${gizmo.author.display_name ? `By ${creatorElement}` : ''}</div></div>
...
function gizmoCardMenu(gizmo, currentUserId) {
  return getGizmosBootstrap(false).then((gizmosBootstrap) => {
    ...
    return `<div id="gizmo-card-menu" class="absolute top-0 right-0 mt-2 mr-2 w-40 rounded-md shadow-lg p-1 bg-gray-700 ring-1 ring-black ring-opacity-5 bg-token-main-surface-primary"> 
    
    ${gizmo.author.user_id === currentUserId ? `<a href="/gpts/editor/${gizmo.id}" target="_self" class="block px-4 py-2 text-sm text-gray-100 hover:bg-token-main-surface-secondary" role="menuitem">Edit</a><button id="delete-gizmo-button" class="block w-full flex items-start px-4 py-2 text-sm text-gray-100 hover:bg-token-main-surface-secondary" role="menuitem">Delete</button>` : ''} 
    
    ${gizmo.share_recipient === 'private' ? '' : `<button id="gizmo-card-${gizmoExistInSidebar ? 'hide-from' : 'add-to'}-sidebar" class="block w-full flex items-start px-4 py-2 text-sm text-gray-100 hover:bg-token-main-surface-secondary" role="menuitem">${gizmoExistInSidebar ? 'Hide from sidebar' : 'Add to sidebar'}</button>`} </div>`;
  });
}
