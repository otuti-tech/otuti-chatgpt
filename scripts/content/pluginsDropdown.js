...
function pluginsDropdown(installedPlugins, enabledPluginIds, idPrefix, forceDark = false) {
  ...
  return `<button id="${idPrefix}-plugins-dropdown-button" class="relative w-full cursor-pointer rounded-md border border-token-border-light bg-token-main-surface-primary pt-1 pl-3 pr-10 text-left focus:border-green-600 focus:outline-none focus:ring-1 focus:ring-green-600 sm:text-sm" type="button">
  <label class="block text-xs text-token-text-primary">Plugins</label>
  <span class="inline-flex w-full truncate font-semibold text-token-text-primary">
    ...
    ${sortedPlugins?.length > 0 ? `<div class="flex gap-1">
      ${sortedPlugins?.map((enabledPlugin) => `<div class="relative bg-token-main-surface-primary hover:bg-token-main-surface-secondary" style="width: 16px; height: 16px;"><img src="${enabledPlugin?.manifest.logo_url}" alt="${enabledPlugin.manifest.name_for_human} logo" class="h-full w-full rounded-sm"><div class="absolute inset-0 ring-1 ring-inset ring-black/10 rounded-sm"></div></div>`).join('')}
...
<ul id="${idPrefix}-plugin-list-dropdown" style="width:400px;max-height:400px" class="hidden absolute z-10 mt-1 overflow-auto rounded-md py-1 text-base ring-1 ring-opacity-5 focus:outline-none ${forceDark ? 'ring-white/20 last:border-0' : ''} bg-token-main-surface-primary dark:ring-white/20 dark:last:border-0 sm:text-sm -translate-x-1/4" role="menu" aria-orientation="vertical" aria-labelledby="plugins-dropdown-button" tabindex="-1">
  <div id="enabled-plugins-limit" class="flex h-8 items-center justify-center border-b border-black/10 bg-gray-50 text-xs ${forceDark ? 'text-white bg-[#272832]' : 'text-gray-800 bg-gray-50'} border-token-border-light dark:bg-[#272832] dark:text-white transition-colors duration-300">${enabledPluginIds?.length || 0}/3 Enabled</div>
  ${createPluginsDropDown(installedPlugins, enabledPluginIds, idPrefix, forceDark)}
  <li id="plugin-store-button" class="group relative flex h-[42px] cursor-pointer select-none items-center overflow-hidden border-b border-black/10 pl-3 pr-9 last:border-0 ${forceDark ? '' : 'border-b'} border-token-border-light text-gray-900 bg-token-main-surface-primary hover:bg-token-main-surface-secondary" role="option" tabindex="-1" aria-selected="false">
  <div class="text-token-text-primary">Plugin store (CMD/CTRL + SHIFT + P)</div>
  <span class="absolute inset-y-0 right-0 flex items-center pr-3 text-token-text-primary">
...
<div id="${idPrefix}-plugin-specifics" style="width: 260px; min-height: 240px; top:50px; left: 405px;"  class="hidden rounded ring-1 ring-black/10 absolute z-[17] -ml-[1px] flex flex-col gap-2 p-3 sm:p-4 ${forceDark ? 'ring-white/20 last:border-0' : ''} bg-token-main-surface-primary dark:ring-white/20 dark:last:border-0">
asd
</div>`;
}
function createPluginsDropDown(installedPlugins, enabledPluginIds, idPrefix, forceDark = false) {
  ...
  return `${sortedPlugins?.map((installedPlugin) => `<li class="text-gray-900 group relative cursor-pointer select-none border-b py-1 pl-3 pr-9 last:border-0 border-token-border-light bg-token-main-surface-primary hover:bg-token-main-surface-secondary" id="${idPrefix}-plugins-dropdown-option-${installedPlugin.id}" role="option" tabindex="-1">
  <span id="${idPrefix}-plugin-dropdown-option-${installedPlugin.id}" class="flex items-center gap-1.5 truncate">
    ...
    <span id="plugin-name-${installedPlugin.id}" class="flex h-6 items-center gap-1 text-token-text-primary">${installedPlugin.manifest.name_for_human}</span>
  ...
  <span id="${idPrefix}-plugin-checkmark-${installedPlugin.id}" class="absolute inset-y-0 right-0 flex items-center pr-3 text-token-text-primary">
      ...
      chrome.storage.local.get(['installedPlugins', 'enabledPluginIds'], ({
        ...
        if (enabledPluginIds?.includes(pluginId)) {
          ...
          if (enabledPluginIds?.length >= 3) {
            ...
            enabledPluginLimit.classList = `flex h-8 flex-shrink-0 items-center justify-center border-b ${forceDark ? 'bg-red-200 text-red-800' : 'bg-red-200 text-red-800'} text-xs border-token-border-light transition-colors duration-300 dark:bg-red-200 dark:text-red-800`;
            setTimeout(() => {
              enabledPluginLimit.classList = `flex h-8 items-center justify-center border-b border-black/10 bg-gray-50 text-xs ${forceDark ? 'text-white bg-[#272832]' : 'text-gray-800 bg-gray-50'} border-token-border-light dark:bg-[#272832] dark:text-white transition-colors duration-300`;
            }, 500);
          } else {
    ...
    if (e.enabledPluginIds && JSON.stringify(e.enabledPluginIds.newValue) !== JSON.stringify(e.enabledPluginIds.oldValue)) {
      chrome.storage.local.get(['installedPlugins'], ({
        ...
        allPluginCheckmarkWrappers.forEach((pluginCheckmarkWrapper) => {
          pluginCheckmarkWrapper.innerHTML = `<div class="flex h-6 w-6 items-center justify-center rounded-full border transition-colors border-token-border-light" aria-hidden="true">
          <svg stroke="currentColor" fill="none" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 h-3 w-3 transition-opacity opacity-0 group-hover:opacity-50" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
            <polyline points="20 6 9 17 4 12"></polyline>
          </svg>
          </div>`;
