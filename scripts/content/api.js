...
function stopConversationWS() {
  return chrome.storage.local.get(['websocket']).then((res) => {
    const { pathname } = new URL(window.location.toString());
    const convIdFromUrl = pathname.split('/c/').pop().replace(/[^a-z0-9-]/gi, '');
    const conversationId = convIdFromUrl;
    if (!conversationId || !res.websocket?.websocket_request_id) {
      return;
    }
    chrome.storage.sync.get(['accessToken']).then((result) => fetch('https://chat.openai.com/backend-api/stop_conversation', {
      method: 'POST',
      headers: {
        ...defaultHeaders,
        Authorization: result.accessToken,
      ...
      body: JSON.stringify({
        ...
        websocket_request_id: res.websocket?.websocket_request_id,
      }),
    }).then((response) => {
      if (response.ok) {
        return response.json();
