...
function addPinNav(sortedNodes) {
  chrome.storage.local.get(['settings'], (res) => {
    ...
    pinNav.classList =
      'flex flex-col items-center py-4 mr-4 absolute right-0 top-0 z-50 justify-center overflow-y-scroll';
    ...
    sortedNodes.forEach((node) => {
      ...
      pin.style =
        'background-color: transparent; border: none; cursor: pointer;width:100%;width: 18px; margin-bottom:4px;transition: width 0.2s ease-in-out;';
      pin.id = `pin-nav-item-${id}`;
      pin.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path fill="gold" d="M48 0H336C362.5 0 384 21.49 384 48V487.7C384 501.1 373.1 512 359.7 512C354.7 512 349.8 510.5 345.7 507.6L192 400L38.28 507.6C34.19 510.5 29.32 512 24.33 512C10.89 512 0 501.1 0 487.7V48C0 21.49 21.49 0 48 0z"/></svg>';
  ...
  chrome.storage.local.get(
    ...
    ({ settings, models, unofficialModels, customModels, enabledPluginIds }) => {
      ...
      if (selectedModel.slug.includes('gpt-4')) {
        // addArkoseScript();
        console.log(`addArkoseScript() Was called, but was removed by Yason`);
      }
      ...
      chrome.storage.local.set(
        {
          settings: {
            ...settings,
            selectedModel,
            selectedLanguage: languageList.find(
              (language) => language.code === languageCode || language.code === 'default',
            ),
            selectedTone: toneList.find(
              (tone) => tone.code === toneCode || tone.code === 'default',
            ),
            selectedWritingStyle: writingStyleList.find(
              (writingStyle) =>
                writingStyle.code === writingStyleCode || writingStyle.code === 'default',
            ),
          },
        ...
        () => {
          ...
          document
            .querySelector(`#language-list-dropdown li#language-selector-option-${languageCode}`)
            ?.click();
          document
            .querySelector(`#tone-list-dropdown li#tone-selector-option-${toneCode}`)
            ?.click();
          document
            .querySelector(
              `#writing-style-list-dropdown li#writing-style-selector-option-${writingStyleCode}`,
            )
            ?.click();
        },
      );
    ...
    chrome.storage.local.get(['conversations', 'settings', 'models'], (res) => {
      ...
      for (let i = 0; i < sortedNodes.length; i += 1) {
        ...
        if (message.role === 'assistant' || message.author?.role === 'assistant') {
          ...
          while (
            nextMessage &&
            nextMessage.recipient === 'all' &&
            (nextMessage.role === 'assistant' || nextMessage.author?.role === 'assistant')
          ) {
      ...
      while (
        messageWrapper.nextElementSibling &&
        messageWrapper.nextElementSibling.id.startsWith('message-wrapper-')
      ) {
      ...
      updateModel(
        sortedNodes[sortedNodes.length - 1].message?.metadata?.model_slug,
        fullConversation,
      );
      updateTotalCounter();
    });
...
function loadConversation(conversationId, searchValue = '', focusOnInput = true) {
  ...
  chrome.storage.sync.get(['name', 'avatar', 'conversationsOrder'], (result) => {
    chrome.storage.local.get(['conversations', 'settings', 'models'], (res) => {
      ...
      const { settings } = res;
      const folderConatainingConversation = conversationsOrder.find((folder) =>
        folder?.conversationIds?.includes(conversationId?.slice(0, 5)),
      );
      let folderName = '';
      ...
      for (let i = 0; i < sortedNodes.length; i += 1) {
        ...
        if (
          ...
          (message.role === 'assistant' || message.author?.role === 'assistant')
        ) {
          let nextMessage = sortedNodes[i + 1]?.message;
          while (
            nextMessage &&
    ...
    button.addEventListener('click', () => {
      chrome.storage.local.get(['conversations', 'settings', 'models'], (result) => {
        ...
        saveButton.addEventListener('click', () => {
          ...
          let parentId = Object.values(conversation.mapping).find(
            (m) => m?.message?.author?.role === 'system',
          )?.id;
          ...
          // messageWrapper.remove();
          const threadCountWrapper = messageWrapper.querySelector(
            `#thread-count-wrapper-${messageId}`,
          );
          ...
          threadCountWrapper.innerText = `${parseInt(childCount, 10) + 1} / ${
            parseInt(childCount, 10) + 1
          }`;
          const threadButtonsWrapper = messageWrapper.querySelector(
            `#thread-buttons-wrapper-${messageId}`,
          );
          ...
          const theOriginalPrompt =
            conversation.mapping[messageId].message.content.parts.join('\n');
  ...
  resultCopyButtons.forEach((btn) => {
    ...
    button.addEventListener('click', () => {
      chrome.storage.local.get(['conversations', 'settings'], (result) => {
        ...
        const text = `${
          result.settings.copyMode ? `>> USER: ${parentMessage}\n>> ASSISTANT: ` : ''
        }${element.innerText}`;
  ...
  threadPrevButtons.forEach((btn) => {
    ...
    button.addEventListener('click', () => {
      chrome.storage.local.get(['conversations'], (result) => {
        ...
        const [currentThreadIndex] = threadButtonsWrapper.textContent
          .split(' / ')
          .map((n) => parseInt(n, 10));
  ...
  threadNextButtons.forEach((btn) => {
    ...
    button.addEventListener('click', () => {
      chrome.storage.local.get(['conversations'], (result) => {
        ...
        const siblings = parent.children;
        const threadButtonsWrapper = document.querySelector(`#thread-count-wrapper-${messageId}`);
        const [currentThreadIndex, threadCount] = threadButtonsWrapper.textContent
          .split(' / ')
          .map((n) => parseInt(n, 10));
        ...
        chrome.storage.local.set({ conversations }, () => {
          ...
          if (settings.showPinNav) {
            ...
            if (isPinned) {
              ...
              const newMessagePinButtons = document.querySelectorAll(
                '[id^="message-pin-button-"].visible',
              );
              // fake because it only has id and pinned=true which is enough for addPinNav
              const fakeSortedNodes = [...newMessagePinButtons].map((b) => ({
                id: b.id.split('message-pin-button-').pop(),
                pinned: true,
              }));
