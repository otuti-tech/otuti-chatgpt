...
(function (global) {
  ...
  const miniumFontSize = (function getMiniumFontSize() {
    if (!isSupported) {
      return;
    }
  ...
  const WordCloud = function WordCloud(elements, options) {
    ...
    let ngx; let ngy; // width and height of the grid
    ...
    switch (settings.color) {
      case 'random-dark':
        getTextColor = function getRandomDarkColor() {
          return `rgb(${Math.floor(Math.random() * 128).toString(10)},${Math.floor(Math.random() * 128).toString(10)},${Math.floor(Math.random() * 128).toString(10)})`;
        };
        break;
    ...
    const getInfoGridFromMouseEvent = function getInfoGridFromMouseEvent(evt) {
      const canvas = evt.currentTarget;
      const rect = canvas.getBoundingClientRect();
      const eventX = evt.clientX - rect.left;
      const eventY = evt.clientY - rect.top;
    ...
    const wordcloudhover = function wordcloudhover(evt) {
      const info = getInfoGridFromMouseEvent(evt);
    ...
    const sendEvent = function sendEvent(type, cancelable, detail) {
      if (cancelable) {
    ...
    const start = function start() {
      ...
      var timer = loopingFunction(function loop() {
        ...
        const canceled = !sendEvent('wordclouddrawn', true, { item: settings.list[i], drawn });
  ...
  if (typeof global.define === 'function' && global.define.amd) {
    global.define('wordcloud', [], () => WordCloud);
